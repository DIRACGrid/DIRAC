...
class MigrationMonitoringHandler(RequestHandler):
  ...
  def export_setMigratingReplicaStatus(self, fileIDs, status):
    ...
    """
    try:
      gLogger.info("setMigratingReplicaStatus: Attempting to update status of %d replicas to '%s'." % (len(fileIDs),
                                                                                                       status))
  ...
  def export_getSummaryWeb(self, selectDict, sortList, startItem, maxItems):
    ...
    if sortList:
      ...
      orderAttribute = None
    res = database.selectMigratingReplicasRows(selectDict, orderAttribute=orderAttribute, 
    ...
    for tup in summaryRowList:
      lfn, pfn, size, se, guid, checksum, submit, complete, status = tup
