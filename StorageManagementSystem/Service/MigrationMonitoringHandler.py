...
from types                                                    import ListType, TupleType, StringType, IntType, DictType
...
class MigrationMonitoringHandler(RequestHandler):
  ...
  def export_getSummaryWeb(self, selectDict, sortList, startItem, maxItems):
    ...
    if sortList:
      ...
      orderAttribute = None
    res = database.selectMigratingReplicasRows(selectDict, orderAttribute=orderAttribute, newer=startDate, 
                                               older=endDate )
    if not res['OK']:
