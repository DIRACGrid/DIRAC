  ...
  def export_getServicesStatusWeb(self, selectDict, sortList, startItem, maxItems):
    """ get present services status list, for the web
        ...
        :Params:
          selectDict = 
            {
              'ServiceName':'name of a service' --- present status
              'ExpandServiceHistory':'name of a service' --- service status history
            }
        ...
        :return:
        {
          ...
          'rpcStub': XX, 'getServicesStatusWeb', ({}, [], X, X)), 

          Value': 
          {

            'ParameterNames': ['ServiceName', 'Tier', 'GridType', 'Country', 'Status', 'DateEffective', 'FormerStatus', 'Reason', 'StatusInTheMask'], 
...
#############################################################################
  ...
  def export_getServicesHistory(self, service):
    ...
    try:
      ...
      try:
        res = rsDB.getServicesHistory(service)
      except RSSDBException, x:
        gLogger.error(whoRaised(x))
      except RSSException, x:
        gLogger.error(whoRaised(x))
