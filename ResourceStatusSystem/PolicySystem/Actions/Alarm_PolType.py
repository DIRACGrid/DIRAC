...
class AlarmPolType(object):
  def __init__(self, name, res, statusType, nc, setup, rsClient, rmDB, **kwargs):
    self.name = name
    self.res = res
      ...
      for notification in NOTIF_D:
        for user in notification['Users']:
          ...
          if 'Mail' in notification['Notifications']:
            histGetter = getattr( self.rsClient, 'get%ssHistory' % granularity )
...
""" % (granularity, self.name, self.res['Status'], self.res['Reason'], was[0], was[1], was[2], self.setup)
            ...
            self.nc.sendMail(self.rmDB.registryGetMailFromLogin(user),
