...
class TestMQProducer( unittest.TestCase ):
  def setUp( self ):
    dest1 = {"name":"/queue/FakeQueue", "publishers":4, "subscribers":[1,2,4]}
    dest2 = {"name":"/queue/test2", "publishers":2, "subscribers":[1,2]}
    conn1 = {"MQConnector": FakeMQConnector(), "destinations":[dest1,dest2]}
...
class TestMQProducer_close( TestMQProducer):
  def test_success( self ):
    ...
    self.assertTrue(result['OK'])
    self.assertEqual(result['Value'], "FakeMQConnection disconnecting")
