...
class StorageElementItem( object ):
  ...
  def isValid( self, operation = '' ):
    ...
    if ( not operation ) and ( not reading ) and ( not writing ) and ( not checking ):
      ...
      return S_ERROR( DError( errno.EACCES, "SE.isValid: Read, write and check access not permitted." ) )
    ...
    if ( operation in self.readMethods ) or ( operation.lower() in ( 'read', 'readaccess' ) ):
      ...
      return S_ERROR( DError( DIRACError.ENOMETH , "SE.isValid: The supplied operation is not known." ) )
    ...
    if operation == 'CheckAccess':
      if not reading:
        if not checking:
          ...
          return S_ERROR( DError( errno.EACCES, "SE.isValid: Check access not currently permitted." ))
    if operation == 'ReadAccess':
      if not reading:
        log.debug( "Read access not currently permitted." )
        return S_ERROR( DError( errno.EACCES, "SE.isValid: Read access not currently permitted." ))
    if operation == 'WriteAccess':
      if not writing:
        log.debug( "Write access not currently permitted." )
        return S_ERROR( DError( errno.EACCES, "SE.isValid: Write access not currently permitted." ))
    if operation == 'RemoveAccess':
      if not removing:
        log.debug( "Remove access not currently permitted." )
        return S_ERROR( DError( errno.EACCES, "SE.isValid: Remove access not currently permitted." ))
  ...
  def __executeMethod( self, lfn, *args, **kwargs ):
    ...
    for storage in self.storages:
      ...
      if not len( urlDict ):
        ...
        if not fcn:
          return S_ERROR( DError( DIRACError.ENOMETH, "SE.__executeMethod: unable to invoke %s, it isn't a member function of storage" ) )
