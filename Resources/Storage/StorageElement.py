...
class StorageElementItem( object ):
  ...
  def isValid( self, operation = '' ):
    ...
    if ( operation in self.readMethods ) or ( operation.lower() in ( 'read', 'readaccess' ) ):
      ...
      return DError( DIRACError.ENOMETH , "SE.isValid: The supplied operation is not known." )
  ...
  def __executeMethod( self, lfn, *args, **kwargs ):
    ...
    for storage in self.storages:
      ...
      if not len( urlDict ):
        ...
        if not fcn:
          return DError( DIRACError.ENOMETH, "SE.__executeMethod: unable to invoke %s, it isn't a member function of storage" )
