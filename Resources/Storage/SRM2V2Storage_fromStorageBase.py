""" :mod: SRM2V2Storage_fromStorageBase.
    =================

    .. module: python
    :synopsis: SRM v2 interface to StorageElement using gfal2. Inherits from StorageBase.
               Most of this module is now in GFAL2StorageBase which serves as new base class.
"""
# # imports
import os
import datetime
...
class SRM2V2Storage_fromStorageBase( StorageBase ):
  """ .. class:: SRM2V2Storage_fromStorageBase
  ...
  def __init__( self, storageName, parameters ):
    ...
    self.log = gLogger.getSubLogger( "SRM2V2Storage_fromStorageBase", True )
#     os.environ['GLOBUS_THREAD_MODEL'] = "pthread"
    ...
    if self.checksumType:
      if str( self.checksumType ).upper() in self.checksumTypes:
        gLogger.debug( "SRM2V2Storage_fromStorageBase: will use %s checksum check" % self.checksumType )
        self.checksumType = self.checksumTypes[ self.checksumType.upper() ]
      else:
        gLogger.warn( "SRM2V2Storage_fromStorageBase: unknown checksum type %s, checksum check disabled" )
        # # GFAL_CKSM_NONE
        self.checksumType = 0
    else:
      # # invert and get name
      self.log.debug( "SRM2V2Storage_fromStorageBase: will use %s checksum" % dict( zip( self.checksumTypes.values(),
  ...
  def exists( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.exists: Checking the existence of %s path(s)" % len( urls ) )
  ...
  def __singleExists( self, path ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase._singleExists: Determining whether %s exists or not" % path )
    ...
    try:
      ...
      self.log.debug( "SRM2V2Storage_fromStorageBase.__singleExists: path exists" )
      return S_OK( True )
    except gfal2.GError, e:
      if e.code == errno.ENOENT:
        errStr = "SRM2V2Storage_fromStorageBase.__singleExists: Path does not exist"
        self.log.debug( errStr )
        return  S_OK( False )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__singleExists: Failed to determine existence of path"
...
### methods for manipulating files ###
  ...
  def isFile( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.isFile: checking whether %s path(s) are file(s)." % len( urls ) )
  ...
  def __isSingleFile( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__isSingleFile: Determining whether %s is a file or not." % path )
    ...
    try:
      ...
      if ( S_ISREG( statInfo.st_mode ) ):  # alternatively drop if/else and just return S_OK ( S_ISREG( statInfo.st_mode ) ) but that's not really readable and we can't write the log.
        ...
        self.log.debug( "SRM2V2Storage_fromStorageBase.__isSingleFile: Path is not a file" )
        return S_OK( False )
    except gfal2.GError, e:
      if e.code == errno.ENOENT:
        errStr = "SRM2V2Storage_fromStorageBase.__isSingleFile: File does not exist."
        self.log.debug( errStr, e.message )
        return S_ERROR( errStr )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__isSingleFile: failed to determine if path %s is a file." % path
  ...
  def putFile( self, path, sourceSize = 0 ):
    ...
    for dest_url, src_file in urls.items():
      if not src_file:
        errStr = "SRM2V2Storage_fromStorageBase.putFile: Source file not set. Argument must be a dictionary (or a list of a dictionary) {url : local path}"
        self.log.debug( errStr )
        return S_ERROR( errStr )
  ...
  def __putSingleFile( self, src_file, dest_url, sourceSize ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__putSingleFile: trying to upload %s to %s" % ( src_file, dest_url ) )
    ...
    if not src_file:
      errStr = 'SRM2V2Storage_fromStorageBase.__putSingleFile: no source defined, please check argument format { destination : localfile }'
      return S_ERROR( errStr )
    else:
      ...
      if src_file.startswith( 'srm' ):
        src_url = src_file
        if not sourceSize:
          errStr = "SRM2V2Storage_fromStorageBase.__putFile: For file replication the source file size in bytes must be provided."
    ...
    # file is local so we can set the protocol and determine source size accordingly
        ...
        if not os.path.exists( src_file ) or not os.path.isfile( src_file ):
          errStr = "SRM2V2Storage_fromStorageBase.__putFile: The local source file does not exist or is a directory"
          self.log.error( errStr, src_file )
          return S_ERROR( errStr )
        src_url = 'file://%s' % src_file
        sourceSize = getSize( src_file )
        if sourceSize == -1:
          errStr = "SRM2V2Storage_fromStorageBase.__putFile: Failed to get file size"
          self.log.error( errStr, src_file )
          return S_ERROR( errStr )
        if sourceSize == 0:
          errStr = "SRM2V2Storage_fromStorageBase.__putFile: Source file size is zero."
          self.log.error( errStr, src_file )
          return S_ERROR( errStr )
    ...
    if not res['OK']:
      errStr = "SRM2V2Storage_fromStorageBase.__putSingleFile: Failed to create destination folder %s" % dest_path
    ...
    try:
      ...
      # no checksum check, compare file sizes for verfication
        ...
        if res['OK']:
          ...
          destSize = None
        self.log.debug( 'SRM2V2Storage_fromStorageBase.__putSingleFile: destSize: %s, sourceSize: %s' % ( destSize, sourceSize ) )
        if destSize == sourceSize:
          return S_OK( destSize )
        else:
          self.log.debug( "SRM2V2Storage_fromStorageBase.__putSingleFile: Source and destination file size don't match. Trying to remove destination file" )
          res = self.__removeSingleFile( dest_url )
          if not res['OK']:
            errStr = 'SRM2V2Storage_fromStorageBase.__putSingleFile: Failed to remove destination file: %' % res['Message']
            return S_ERROR( errStr )
          errStr = "SRM2V2Storage_fromStorageBase.__putSingleFile: Source and destination file size don't match. Removed destination file"
      ...
      errStr = "SRM2V2Storage_fromStorageBase.__putSingleFile: Failed to copy file %s to destination url %s: [%d] %s" % ( src_file, dest_url, e.code, e.message )
      return S_ERROR( errStr )
  ...
  def getFile( self, path, localPath = False ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.getFile: Trying to download %s files." % len( urls ) )
  ...
  def __getSingleFile( self, src_url, dest_file ):
    ...
    """
    self.log.info( "SRM2V2Storage_fromStorageBase.__getSingleFile: Trying to download %s to %s" % ( src_url, dest_file ) )

    if not os.path.exists( os.path.dirname( dest_file ) ):
      self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleFile: Local directory does not yet exist. Creating...", os.path.dirname( dest_file ) )
      try:
        os.makedirs( os.path.dirname( dest_file ) )
      except OSError, error:
        errStr = "SRM2V2Storage_fromStorageBase.__getSingleFile: Error while creating the destination folder"
    ...
    if not res['OK']:
      errStr = "SRM2V2Storage_fromStorageBase.__getSingleFile: Error while determining file size: %s" % res['Message']
    ...
    try:
      ...
      if self.checksumType:
        ...
        if destSize == remoteSize:
          ...
          errStr = "SRM2V2Storage_fromStorageBase.__getSingleFile: File sizes don't match. Something went wrong. Removing local file %s" % dest_file
          self.log.error( errStr, {remoteSize : destSize} )
          if os.path.exists( dest_file ):
            os.remove( dest_file )
          return S_ERROR( errStr )
    except gfal2.GError, e:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleFile: Could not copy %s to %s, [%d] %s' % ( src_url, dest, e.code, e.message )
      # errStr = "SRM2V2Storage_fromStorageBase.__getSingleFile: Failed to copy file %s to destination url %s " % ( src_url, dest_file )
  ...
  def removeFile( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.removeFile: Attemping to remove %s files" % len( urls ) )
  ...
  def __removeSingleFile( self, path ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleFile: Attemping to remove single file %s" % path )
    ...
    try:
      ...
      if status == 0:
        self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleFile: File successfully removed" )
        return S_OK( True )
      elif status < 0:
        errStr = 'SRM2V2Storage_fromStorageBase.__removeSingleFile: return status < 0. Error occured.'
        return S_ERROR( errStr )
    except gfal2.GError, e:
      # file doesn't exist so operation was successful
      if e.code == errno.ENOENT:
        errStr = "SRM2V2Storage_fromStorageBase.__removeSingleFile: File does not exist."
        self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleFile: Error while removing file." )
        return S_OK( True )
      elif e.code == errno.EISDIR:
        errStr = "SRM2V2Storage_fromStorageBase.__removeSingleFile: path is a directory."
        self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleFile: Path is a directory." )
        return S_ERROR( errStr )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__removeSingleFile: Failed to remove file."
        self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleFile: Failed to remove file: [%d] %s" % ( e.code, e.message ) )
        return  S_ERROR( errStr )
  ...
  def getFileSize( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.getFileSize: Trying to determine file size of %s files" % len( urls ) )
  ...
  def __getSingleFileSize( self, path ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleFileSize: Determining file size of %s" % path )
    ...
    if not res['Value']:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleFileSize: path is not a file'
      self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleFileSize: path is not a file' )
      return S_ERROR( errStr )
      ...
      try:
        ...
        self.log.debug( "SRM2V2Storage_fromStorageBase.__singleExists: File size successfully determined" )
        return S_OK( long ( statInfo.st_size ) )
      except gfal2.GError, e:
          errStr = "SRM2V2Storage_fromStorageBase.__singleExists: Failed to determine file size."
          self.log.error( errStr, e.message )
          return S_ERROR( errStr )
  ...
  def getFileMetadata( self, path ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.getFileMetadata: trying to read metadata for %s paths' % len( urls ) )
  ...
  def __getSingleFileMetadata( self, path ):
    ...
    """
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleFileMetadata: trying to read metadata for %s' % path )
    ...
    if not metaDict['File']:
      errStr = "SRM2V2Storage_fromStorageBase.__getSingleFileMetadata: supplied path is not a file"
  ...
  def __getSingleMetadata(self, path):
    ...
    """
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleMetadata: reading metadata for %s' % path )
    ...
    try:
      ...
      if e.code == errno.ENOENT:
        errStr = "SRM2V2Storage_fromStorageBase.__getSingleMetadata: Path does not exist"
        self.log.error( errStr, e.message )
        return S_ERROR ( errStr )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__getSingleMetadata: Failed to retrieve metadata from path."
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
  ...
  def prestageFile( self, path, lifetime = 86400 ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.prestageFile: Attempting to issue stage requests for %s file(s).' % len( urls ) )
  ...
  def __prestageSingleFile( self, path, lifetime ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFile: Attempting to issue stage request for single file: %s" % path )

    try:
      self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFile: spaceToken: %s" % self.spaceToken )
      self.gfal2.set_opt_string( "SRM PLUGIN", "SPACETOKENDESC", self.spaceToken )
      ...
      self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFile: Staging issued - Status: %s" % status )
      if status >= 0:
        return S_OK( token )
      else:
        errStr = 'SRM2V2Storage_fromStorageBase.__prestageSingleFile: an error occured while issuing prestaging.'
        return S_ERROR( errStr )
    except gfal2.GError, e:
      errStr = "SRM2V2Storage_fromStorageBase.__prestageSingleFile: Error occured while prestaging file %s. [%d] %s" % ( path, e.code, e.message )
      self.log.error( errStr, e.message )
      return S_ERROR( errStr )
  ...
  def prestageFileStatus( self, path ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.prestageFileStatus: Checking the staging status for %s file(s).' % len( urls ) )
  ...
  def __prestageSingleFileStatus( self, path, token ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: Checking prestage file status for %s" % path )
    ...
    try:
      ...
      if status == 0:
        self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: File not staged" )
        return S_OK( False )
      elif status == 1:
        self.log.debug( "SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: File is staged" )
        return S_OK( True )
      else:
        errStr = 'SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: an error occured while checking prestage status.'
        return S_ERROR( errStr )
    except gfal2.GError, e:
      if e.code == errno.EAGAIN:
        ...
        errStr = 'SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: Polling request timed out'
        self.log.debug( errStr )
        return S_ERROR( errStr )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__prestageSingleFileStatus: Error occured while polling for prestaging file %s. [%d] %s" % ( path, e.code, e.message )
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
  ...
  def pinFile( self, path, lifetime = 86400 ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.pinFile: Attempting to pin %s file(s).' % len( urls ) )
    failed = {}
    successful = {}
    for url in urls:
  ...
  def __pinSingleFile( self, path, lifetime ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__pinSingleFile: Attempting to issue pinning request for single file: %s" % path )

    try:
      self.log.debug( "SRM2V2Storage_fromStorageBase.__pinSingleFile: spaceToken: %s" % self.spaceToken )
      self.gfal2.set_opt_string( "SRM PLUGIN", "SPACETOKENDESC", self.spaceToken )
      self.gfal2.set_opt_string_list( "SRM PLUGIN", "TURL_PROTOCOLS", self.defaultLocalProtocols )
      self.gfal2.set_opt_boolean( "BDII", "ENABLE", True )
      self.gfal2.set_opt_integer( "SRM PLUGIN", "OPERATION_TIMEOUT", self.gfal2Timeout )
      ( status, token ) = self.gfal2.bring_online( path, lifetime, self.stageTimeout, True )
      self.log.debug( "SRM2V2Storage_fromStorageBase.__pinSingleFile: pinning issued - Status: %s" % status )
      if status >= 0:
        return S_OK( token )
      else:
        errStr = 'SRM2V2Storage_fromStorageBase.__pinSingleFile: an error occured while issuing pinning.'
        return S_ERROR( errStr )
    except gfal2.GError, e:
      errStr = "SRM2V2Storage_fromStorageBase.__pinSingleFile: Error occured while pinning file %s. [%d] %s" % ( path, e.code, e.message )
      self.log.error( errStr )
      return S_ERROR( errStr )
  ...
  def releaseFile( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.releaseFile: Attempting to release %s file(s)." % len( urls ) )
  ...
  def __releaseSingleFile( self, path, token ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__releaseSingleFile: Attempting to release single file: %s" % path )
    if not type( token ) == StringType:
      token = str( token )
    try:
      self.log.debug( "SRM2V2Storage_fromStorageBase.__releaseSingleFile: spaceToken: %s" % self.spaceToken )
      self.gfal2.set_opt_string( "SRM PLUGIN", "SPACETOKENDESC", self.spaceToken )
      self.gfal2.set_opt_string_list( "SRM PLUGIN", "TURL_PROTOCOLS", self.defaultLocalProtocols )
      self.gfal2.set_opt_boolean( "BDII", "ENABLE", True )
      self.gfal2.set_opt_integer( "SRM PLUGIN", "OPERATION_TIMEOUT", self.gfal2Timeout )
      ...
      if status >= 0:
        ...
        errStr = "SRM2V2Storage_fromStorageBase.__releaseSingleFile: Error occured: Return status < 0"
        return S_ERROR( errStr )
    except gfal2.GError, e:
      errStr = "SRM2V2Storage_fromStorageBase.__releaseSingleFile: Error occured while releasing file %s. [%d] %s" % ( path, e.code, e.message )
      self.log.error( errStr )
      return S_ERROR( errStr )
  ...
  def __getChecksum( self, path, checksumType = 'ADLER32' ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getChecksum: Trying to calculate checksum of file %s' % path )
    res = self.__isSingleFile( path )

    if not res['OK']:
      errStr = 'SRM2V2Storage_fromStorageBase.__getChecksum: provided path is not a file'
      self.log.error( errStr, path )
      return S_ERROR( errStr )
    else:
      try:
        ...
        errStr = 'SRM2V2Storage_fromStorageBase.__getChecksum: failed to calculate checksum.'
        self.log.error( errStr, e.message )
...
### methods for manipulating directories ###
  ...
  def createDirectory( self, path ):
    ...
    for url in urls:
      ...
      if res['OK']:
        self.log.debug( "SRM2V2Storage_fromStorageBase.createDirectory: Successfully created directory on storage %s" % url )
        successful[url] = True
      else:
        self.log.error( "SRM2V2Storage_fromStorageBase.createDirectory: Failed to create directory on storage.", "%s: %s" % ( url, res['Message'] ) )
        failed[url] = res['Message']
    return S_OK( { 'Failed' : failed, 'Successful' : successful } )
...
#         path = key
#       self.log.debug( "SRM2V2Storage_fromStorageBase.__createSingleDirectory: Attempting to create directory %s." % path )
    ...
    try:
      ...
      self.log.debug( 'SRM2V2Storage_fromStorageBase.__createSingleDirectory: Status return of mkdir: %s' % status )
      if status >= 0:
        return S_OK()
      else:
        errStr = 'SRM2V2Storage_fromStorageBase.__createSingleDirectory: Status return > 0. Error.'
        return S_ERROR( errStr )
    except gfal2.GError, e:
      # error: directory already exists
      if e.code == errno.EEXIST:
        self.log.debug( "SRM2V2Storage_fromStorageBase.__createSingleDirectory: Directory already exists" )
        return S_OK()
      # any other error: failed to create directory
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__createSingleDirectory: failed to create directory."
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
  ...
  def isDirectory( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.isDirectory: checking whether %s path(s) are directory(ies)." % len( urls ) )
  ...
  def __isSingleDirectory( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__isSingleDirectory: Determining whether %s is a directory or not." % path )
    try:
      self.gfal2.set_opt_boolean( "BDII", "ENABLE", False )
      self.gfal2.set_opt_integer( "SRM PLUGIN", "OPERATION_TIMEOUT", self.gfal2Timeout )
      statInfo = self.gfal2.stat( path )
      if ( S_ISDIR( statInfo.st_mode ) ):  # alternatively drop if/else and just return S_OK ( S_OK ( S_ISDIR( statInfo.st_mode ) ) but that's not really readable and we can't write the log.
        ...
        self.log.debug( "SRM2V2Storage_fromStorageBase.__isSingleDirectory: Path is not a directory" )
        return S_OK( False )
    except gfal2.GError, e:
      if e.code == errno.ENOENT:
        errStr = "SRM2V2Storage_fromStorageBase.__isSingleDirectory: Directory doesn't exist."
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
      else:
        errStr = "SRM2V2Storage_fromStorageBase.__isSingleDirectory: failed to determine if path %s is a directory." % path
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
  ...
  def listDirectory( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.listDirectory: Attempting to list %s directories" % len( urls ) )
    self.log.debug( 'SRM2V2Storage_fromStorageBase.listDirectory: Directories to list: %s' % urls )
    ...
    for url, isDirectory in res['Value']['Successful'].items():
      if isDirectory:
        ...
        errStr = "SRM2V2Storage_fromStorageBase.listDirectory: path is not a directory"
  ...
  def __listSingleDirectory( self, path ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__listSingleDirectory: Attempting to list content of single directory" )
    ...
    try:
      ...
      for entry in listing:
        ...
        self.log.debug( 'SRM2V2Storage_fromStorageBase.__listSingleDirectory: path: %s' % fullPath )
        ...
        if metadataDict['Directory']:
          ...
          self.log.debug( "SRM2V2Storage_fromStorageBase.__listSingleDirectory: found item which is neither file nor directory", fullPath )

      return S_OK( {'SubDirs' : subDirs, 'Files' : files} )

    except gfal2.GError, e:
      if e.code == errno.ENOENT:
        errStr = 'SRM2V2Storage_fromStorageBase.__listSingleDirectory: directory does not exist'
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
      else:
        errStr = 'SRM2V2Storage_fromStorageBase.__listSingleDirectory: could not list directory content.'
        self.log.error( errStr, e.message )
        return S_ERROR( errStr )
  ...
  def getDirectory( self, path, localPath = False ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.getDirectory: Attempting to get local copies of %s directories. %s" % ( len( urls ), urls ) )
    ...
    for src_dir in urls:
      ...
      if res['OK']:
        if res['Value']['AllGot']:
          self.log.debug( "SRM2V2Storage_fromStorageBase.getDirectory: Successfully got local copy of %s" % src_dir )
          successful[src_dir] = {'Files':res['Value']['Files'], 'Size':res['Value']['Size']}
        else:
          self.log.error( "SRM2V2Storage_fromStorageBase.getDirectory: Failed to get entire directory.", src_dir )
          failed[src_dir] = {'Files':res['Value']['Files'], 'Size':res['Value']['Size']}
      else:
        self.log.error( "SRM2V2Storage_fromStorageBase.getDirectory: Completely failed to get local copy of directory.", src_dir )
  ...
  def __getSingleDirectory( self, src_dir, dest_dir ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleDirectory: Attempting to download directory %s at %s" % ( src_dir, dest_dir ) )
    ...
    if not res['OK']:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: Failed to find the source directory'
      self.log.debug( res['Message'], src_dir )
      return S_ERROR( errStr )

    # res['Value'] is False if it's not a directory
    if not res['Value']:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: The path provided is not a directory'
    ...
    if not os.path.exists( dest_dir ):
      try:
        ...
        errStr = 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: Error trying to create destination directory %s' % error
    ...
    if not res['OK']:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: Failed to list the source directory.'
      self.log.error( errStr, src_dir )
      return S_ERROR( errStr )
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: Trying to download the %s files' % len( sFilesDict ) )
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleDirectory: Trying to recursively download the %s directories' % len( subDirsDict ) )
  ...
  def putDirectory( self, path ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.putDirectory: Attempting to put %s directories to remote storage' % len( urls ) )
    ...
    for destDir, sourceDir in urls.items():
      if not sourceDir:
        ...
        errStr = 'SRM2V2Storage_fromStorageBase.putDirectory: No source directory set, make sure the input format is correct { dest. dir : source dir }'
        return S_ERROR( errStr )
      res = self.__putSingleDirectory( sourceDir, destDir )
      if res['OK']:
        if res['Value']['AllPut']:
          self.log.debug( "SRM2V2Storage_fromStorageBase.putDirectory: Successfully put directory to remote storage: %s" % destDir )
          successful[destDir] = { 'Files' : res['Value']['Files'], 'Size' : res['Value']['Size']}
        else:
          self.log.error( "SRM2V2Storage_fromStorageBase.putDirectory: Failed to put entire directory to remote storage.", destDir )
          failed[destDir] = { 'Files' : res['Value']['Files'], 'Size' : res['Value']['Size']}
      else:
        self.log.error( "SRM2V2Storage_fromStorageBase.putDirectory: Completely failed to put directory to remote storage.", destDir )
  ...
  def __putSingleDirectory( self, src_directory, dest_directory ):
    ...
    """
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__putSingleDirectory: trying to upload %s to %s' % ( src_directory, dest_directory ) )

    filesPut = 0
    sizePut = 0
    
    if not os.path.isdir( src_directory ):
      errStr = 'SRM2V2Storage_fromStorageBase.__putSingleDirectory: The supplied source directory does not exist or is not a directory.'
    ...
    for fileName in contents:
      ...
      # localPath is another folder, start recursion
        ...
        if not res['OK']:
          errStr = 'SRM2V2Storage_fromStorageBase.__putSingleDirectory: Failed to put directory to storage.'
    ...
    if directoryFiles:
      ...
      if not res['OK']:
        self.log.error( 'SRM2V2Storage_fromStorageBase.__putSingleDirectory: Failed to put files to storage.', res['Message'] )
        allSuccessful = False
      else:
  ...
  def removeDirectory( self, path, recursive = False ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.removeDirectory: Attempting to remove %s directories." % len( urls ) )
    ...
    for url in urls:
      ...
      if res['OK']:
        if res['Value']['AllRemoved']:
          self.log.debug( "SRM2V2Storage_fromStorageBase.removeDirectory: Successfully removed %s" % url )
          successful[url] = {'FilesRemoved':res['Value']['FilesRemoved'], 'SizeRemoved':res['Value']['SizeRemoved']}
        else:
          self.log.error( "SRM2V2Storage_fromStorageBase.removeDirectory: Failed to remove entire directory.", path )
          failed[url] = {'FilesRemoved':res['Value']['FilesRemoved'], 'SizeRemoved':res['Value']['SizeRemoved']}
      else:
        self.log.error( "SRM2V2Storage_fromStorageBase.removeDirectory: Completely failed to remove directory.", url )
  ...
  def __removeSingleDirectory( self, path, recursive = False ):
    ...
    if not res['OK']:
      errStr = "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: %s" % res['Message']
      self.log.error( errStr, path )
      return S_ERROR( errStr )

    # res['Value'] is True if it is a directory
    if not res['Value']:
      errStr = "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: The supplied path is not a directory."
      self.log.error( errStr, path )
      return S_ERROR( errStr )
    ...
    if not res['OK']:
      errStr = "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Failed to list the directory."
      self.log.error( errStr, path )
      return S_ERROR( errStr )
    ...
    if recursive:
      ...
      self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Trying to recursively remove %s folder." % len( subDirsDict ) )
      for subDir in subDirsDict:
        subDirName = os.path.basename( subDir )
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Trying to remove %s files." % len( sFilesDict ) )
    for sFile in sFilesDict:
    ...
    if ( recursive and allRemoved ) or ( not recursive and removedAllFiles and ( len( subDirsDict ) == 0 ) ):
      try:
        ...
        if status < 0:
          errStr = "SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Error occured while removing directory. Status: %s" % status
          self.log.debug( errStr )
          allRemoved = False
      except gfal2.GError, e:
        if e.code == errno.ENOENT:
          errStr = 'SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Files does not exist'
          self.log.debug( errStr )
        else:
          errStr = 'SRM2V2Storage_fromStorageBase.__removeSingleDirectory: Failed to remove directory %s' % path
          self.log.debug( errStr )
          allRemoved = False
  ...
  def getDirectorySize( self, path ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.getDirectorySize: Attempting to get size of %s directories' % len( urls ) )
  ...
  def __getSingleDirectorySize( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleDirectorySize: Attempting to get the size of directory %s" % path )
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleDirectorySize: Successfully obtained size of %s." % path )
  ...
  def getDirectoryMetadata( self, path ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.getDirectoryMetadata: Attempting to fetch metadata." )
  ...
  def __getSingleDirectoryMetadata( self, path ):
    ...
    """
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getSingleDirectoryMetadata: Fetching metadata of directory %s." % path )
    ...
    if not metadataDict['Directory']:
      errStr = "SRM2V2Storage_fromStorageBase.__getSingleDirectoryMetadata: Provided path is not a directory."
      self.log.error( errStr, path )
      return S_ERROR( errStr )
...
##################################################################
  ...
  def getTransportURL( self, path, protocols = False ):
    ...
    self.log.debug( 'SRM2V2Storage_fromStorageBase.getTransportURL: Attempting to retrieve tURL for %s paths' % len( urls ) )
  ...
  def __getSingleTransportURL( self, path, protocols = False ):
    ...
    """
    self.log.debug( 'SRM2V2Storage_fromStorageBase.__getSingleTransportURL: trying to retrieve tURL for %s' % path )
    if protocols:
    ...
    if res['OK']:
      ...
      if 'user.replicas' in attributeDict.keys():
        ...
        errStr = 'SRM2V2Storage_fromStorageBase.__getSingleTransportURL: Extended attribute tURL is not set.'
        self.log.debug( errStr )
        return S_ERROR( errStr )
    else:
      errStr = 'SRM2V2Storage_fromStorageBase.__getSingleTransportURL: %s' % res['Message']
      return S_ERROR( errStr )
  ...
  def __getProtocols( self ):
    ...
    self.log.debug( "SRM2V2Storage_fromStorageBase.__getProtocols: Trying to get protocols for storage %s." % self.name )
    if not sections['OK']:
      return sections
    ...
    for section in sections['Value']:
      ...
      if gConfig.getValue( path, '' ) == self.protocol:
        ...
        if siteProtocols:
          self.log.debug( 'SRM2V2Storage_fromStorageBase.__getProtocols: Found SE protocols list to override defaults:', ', '.join( siteProtocols, ) )
          protocolsList = siteProtocols

    if not protocolsList:
      self.log.debug( "SRM2V2Storage_fromStorageBase.__getProtocols: No protocols provided, using the default protocols." )
      protocolsList = gConfig.getValue( '/Resources/StorageElements/DefaultProtocols', [] )
      
    # if there is even no default protocol
    if not protocolsList:
      return S_ERROR( "SRM2V2Storage_fromStorageBase.__getProtocols: No local protocols defined and no defaults found." )
