...
class JobPlotter(BaseReporter):
    ...
    def _reportCPUUsed(self, reportRequest):
        selectFields = (
            ...
            reportRequest["groupingFields"][1] + ["startTime", "bucketLength", "CPUTime"],
        )
        retVal = self._getTimedData(
            reportRequest["startTime"],
            reportRequest["endTime"],
    ...
    def _reportNormCPUUsed(self, reportRequest):
        selectFields = (
            self._getSelectStringForGrouping(reportRequest["groupingFields"]) + ", %s, %s, SUM(%s)",
            reportRequest["groupingFields"][1] + ["startTime", "bucketLength", "NormCPUTime"],
        )
        retVal = self._getTimedData(
            reportRequest["startTime"],
            reportRequest["endTime"],
    ...
    def _reportWallTime(self, reportRequest):
        selectFields = (
            self._getSelectStringForGrouping(reportRequest["groupingFields"]) + ", %s, %s, SUM(%s)",
            reportRequest["groupingFields"][1] + ["startTime", "bucketLength", "ExecTime"],
        )
        retVal = self._getTimedData(
            reportRequest["startTime"],
            reportRequest["endTime"],
    ...
    def _reportTotalCPUUsed(self, reportRequest):
        selectFields = (
            self._getSelectStringForGrouping(reportRequest["groupingFields"]) + ", SUM(%s)/86400",
    ...
    def _reportProcessingBandwidth(self, reportRequest):
        selectFields = (
            self._getSelectStringForGrouping(reportRequest["groupingFields"]) + ", %s, %s, SUM((%s)/(%s))/SUM(%s)",
            reportRequest["groupingFields"][1]
            + ["startTime", "bucketLength", "InputDataSize", "CPUTime", "entriesInBucket"],
        )
        retVal = self._getTimedData(
            reportRequest["startTime"],
            reportRequest["endTime"],
    ...
    def _reportHistogramCPUUsed(self, reportRequest):
        selectFields = (
            self._getSelectStringForGrouping(reportRequest["groupingFields"]) + ", %s",
            reportRequest["groupingFields"][1] + ["CPUTime"],
        )

        retVal = self._getBucketData(
            reportRequest["startTime"], reportRequest["endTime"], selectFields, reportRequest["condDict"]
        )
        if not retVal["OK"]:
            return retVal
        dataDict = retVal["Value"]
