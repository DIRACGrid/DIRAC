...
def archiveRequestAndOp(listOfLFNs):
    ...
    op.Arguments = DEncode.encode(
        {
            "SourceSE": switches.get("SourceSE", "SOURCE-SE"),
...
def archiveFiles(mocker, archiveRequestAndOp, multiRetValOK):
    ...
    archiveLFN = "/vo/tars/myTar.tar"
    af.fc.isFile.return_value = S_OK({"Failed": {archiveLFN: "no file"}, "Successful": {}})
...
def test_run_OK(archiveFiles, _myMocker, listOfLFNs):
    archiveFiles._run()
    archiveFiles.dm.getFile.assert_called_with(
        listOfLFNs[9], destinationDir=os.path.join(DEST_DIR, "MyRequest", "vo"), sourceSE="SOURCE-SE"
    )
...
def test_run_Fail(archiveFiles, _myMocker, listOfLFNs):
    ...
    with pytest.raises(RuntimeError, match="Completely failed to download file"):
        archiveFiles._run()
    archiveFiles.dm.getFile.assert_called_with(
        listOfLFNs[5], destinationDir=os.path.join(DEST_DIR, "MyRequest", "vo"), sourceSE="SOURCE-SE"
    )
    for opFile in archiveFiles.operation:
...
def test_uploadTarBall_breaks(archiveFiles, _myMocker, listOfLFNs):
    ...
    archiveFiles.dm.putAndRegister.assert_called_with("/vo/tars/myTar.tar", "myTar.tar", "TARBALL-SE")
...
def test_checkReplicas_success(archiveFiles):
    ...
    archiveFiles.lfns = [opFile.LFN for opFile in archiveFiles.waitingFiles]
    archiveFiles.fc.getReplicas.side_effect = partial(multiRetVal, Index=11, Success={"SOURCE-SE": "PFN"})
...
def test_checkReplicas_notAt(archiveFiles):
    ...
    archiveFiles.parameterDict = {"SourceSE": "SOURCE-SE"}
    archiveFiles.lfns = [opFile.LFN for opFile in archiveFiles.waitingFiles]
    archiveFiles.fc.getReplicas.side_effect = partial(multiRetVal, Index=11, Success={"Not-SOURCE-SE": "PFN"})
...
def test_checkReplicas_noSuchFile(archiveFiles):
    ...
    archiveFiles.parameterDict = {"SourceSE": "SOURCE-SE"}
    archiveFiles.lfns = [opFile.LFN for opFile in archiveFiles.waitingFiles]
    archiveFiles.fc.getReplicas.side_effect = partial(
        multiRetVal, Index=7, Success={"SOURCE-SE": "PFN"}, Error="No such file or directory"
    )
    assert archiveFiles._checkReplicas() is None
...
def test_checkReplicas_somefailed(archiveFiles):
    ...
    archiveFiles.lfns = [opFile.LFN for opFile in archiveFiles.waitingFiles]
    archiveFiles.fc.getReplicas.side_effect = partial(
        multiRetVal, Index=7, Success={"SOURCE-SE": "PFN"}, Error="some error"
    )
...
def test_registerDescendent_success(archiveFiles, listOfLFNs):
    ...
    archiveFiles.fc.addFileAncestors = Mock(
        name="AFA", return_value=S_OK({"Failed": {}, "Successful": {archiveLFN: "Done"}})
    )
    assert archiveFiles._registerDescendent() is None
...
def test_registerDescendent_PartialFailure(archiveFiles, listOfLFNs):
    ...
    archiveFiles.parameterDict = {"RegisterDescendent": True, "ArchiveLFN": archiveLFN}
    archiveFiles.fc.addFileAncestors = Mock(
        name="AFA", side_effect=(S_ERROR("Failure"), S_OK({"Failed": {}, "Successful": {archiveLFN: "Done"}}))
    )
