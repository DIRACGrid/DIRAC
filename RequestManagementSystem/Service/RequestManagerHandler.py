...
class RequestManagerHandler( RequestHandler ):
  ...
  types_readRequests = [StringTypes ]
  @staticmethod
  def export_readRequest( requestName ):
    """ read request given its name """
    try:
      res = gRequestDB.readRequest( requestName )
      return res
    except Exception, error:
      errStr = "RequestManagerHandler.readRequest: Exception while reading request."
      gLogger.exception( errStr, '', lException = error )
      return S_ERROR( errStr )
