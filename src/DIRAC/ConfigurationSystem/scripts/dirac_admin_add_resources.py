...
class DIRACScript(_DIRACScript):
  ...
  def checkUnusedCEs(self):
    ...
    if not result['OK']:
      ...
      DIRACExit(-1)
    self.ceBdiiDict = result['BdiiInfo']

    unknownCEs = result['UnknownCEs']
    if unknownCEs:
      gLogger.notice('There is no (longer) information about the following CEs for the %s VO:' % self.vo)
...
def main(self):
  ...
  self.processScriptSwitches()

  if not self.vo:
    gLogger.error('No VO specified')
    DIRACExit(-1)

  self.vo = getVOOption(self.vo, 'VOMSName', self.vo)
