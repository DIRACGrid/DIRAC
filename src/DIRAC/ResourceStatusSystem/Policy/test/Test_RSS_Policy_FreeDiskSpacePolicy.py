...
class FreeDiskSpacePolicy_Success(FreeDiskSpacePolicy_TestCase):
    ...
    def test_evaluate_fraction_thresholds(self):
        ...
        # Absolute thresholds are set low enough to not trigger
        res = module._evaluate(
            {
                "OK": True,
                "Value": {
        ...
        # Free space is above absolute thresholds and below Degraded_threshold but below fraction threshold (Banned wins)
        res = module._evaluate(
            {
                "OK": True,
                "Value": {
