...
def main(self):
  ...
  if 'status' in switches:
    queryDict['Status'] = str(switches['status'])

  if 'se' in switches:
    queryDict['SE'] = str(switches['se'])
  ...
  if 'limit' in switches:
    gLogger.notice("Query limited to %s entries" % switches['limit'])
    res = client.getCacheReplicas(queryDict, None, newer, None, None, int(switches['limit']))
  else:
