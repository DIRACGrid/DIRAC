...
class DiracAdmin(API):
  ...
  #############################################################################
  def checkProxyUploaded(self, user, userGroup, requiredTime):
    """ Check if a user(DN-group) has a proxy in the proxy management
        ...
        :return: S_OK,S_ERROR
    """
    return gProxyManager.userHasProxy(user, userGroup, requiredTime)
  ...
  #############################################################################
  def getPilotProxy(self, user, userGroup, validity=43200):
    """ Retrieves a pilot proxy with default 12hr validity and stores
        ...
        :return: S_OK,S_ERROR
    """
    return gProxyManager.downloadCorrectProxy(user, userGroup, requiredTimeLeft=validity)
