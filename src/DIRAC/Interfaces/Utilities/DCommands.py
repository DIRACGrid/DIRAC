...
class DConfig:
    ...
    def bootstrapFile(self):
        if not os.path.exists(self.configDir):
            ...
            gLogger.error(f'Error: "{self.configDir}" config dir is not a directory')
            DIRAC.exit(-1)
        elif not os.stat(self.configDir).st_mode != stat.S_IRWXU:
            gLogger.error(f'Error: "{self.configDir}" config dir doesn\'t have correct permissions')
            DIRAC.exit(-1)
...
def sessionFromProxy(config=DConfig(), sessionDir=None):
    proxyPath = _getProxyLocation()
    if not proxyPath:
        gLogger.error("ERROR: No proxy found")
        return None
