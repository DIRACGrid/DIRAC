...
def test_createAndExecuteJobWrapperTemplate_success(extraOptions):
    ...
    assert f"{os.path.join(os.getcwd(), 'job/Wrapper')}" in jobWrapperContent
...
def test_createAndExecuteJobWrapperLightTemplate_success(extraOptions):
    """Test the creation of a light job wrapper and its execution:
    ...
    # 4. We recreate the job wrapper light template with the payload params now
    ...
    # 5. We recreate the job wrapper light template with the payload params and the additional job params
