...
class JobAgent(AgentModule):
    ...
    def execute(self):
        ...
        for param in matcherParams:
            if param not in matcherInfo:
                jobReport.setJobStatus(status="Failed", minorStatus="Matcher did not return %s" % (param))
                return self.__finish("Matcher Failed")
            elif not matcherInfo[param]:
                jobReport.setJobStatus(status="Failed", minorStatus="Matcher returned null %s" % (param))
                return self.__finish("Matcher Failed")
            else:
        ...
        if not parameters["OK"]:
            jobReport.setJobStatus(status="Failed", minorStatus="Could Not Extract JDL Parameters")
        ...
        try:
            ...
            jobReport.setJobStatus(status="Matched", minorStatus="Job Received by Agent", sendFlag=False)
    ...
    def _checkInstallSoftware(self, jobID, jobParams, resourceParams, jobReport):
        ...
        jobReport.setJobStatus(status="Matched", minorStatus="Installing Software", sendFlag=False)
    ...
    def _submitJob(
        ...
        jobReport.setJobStatus(status="Matched", minorStatus="Submitting To CE")
    ...
    def _rescheduleFailedJob(self, jobID, message, stop=True):
        ...
        jobReport.setJobStatus(status="Rescheduled", applicationStatus=message, sendFlag=True)
    ...
    def finalize(self):
        ...
        result = PilotManagerClient().setPilotStatus(
            str(self.pilotReference), "Done", gridCE, "Report from JobAgent", self.siteName, queue
        )
        if not result["OK"]:
