...
class JobStateUpdateHandler(RequestHandler):
    ...
    def export_setJobStatus(cls, jobID, status="", minorStatus="", source="Unknown", datetime=None):
        ...
        status information.
        """
        return cls.__setJobStatus(int(jobID), status=status, minorStatus=minorStatus, source=source, datetime=datetime)
    ...
    def export_setJobsStatus(cls, jobIDs, status="", minorStatus="", source="Unknown", datetime=None):
        ...
        """
        for jobID in jobIDs:
            cls.__setJobStatus(int(jobID), status=status, minorStatus=minorStatus, source=source, datetime=datetime)
        return S_OK()
