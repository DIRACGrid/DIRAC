...
class SystemLoggingDB(DB):
    ...
    def __init__(self):
        ...
        self.tableDict = {
            ...
            "MessageRepository": {
                ...
                "Indexes": {
                    "TimeStampsIDX": ["MessageTime"],
                    "FixTextIDX": ["FixedTextID"],
                    "UserIDX": ["UserDNID"],
                    "IPsIDX": ["ClientIPNumberID"],
                },
                "ForeignKeys": {
                    "UserDNID": "UserDNs.UserDNID",
    ...
    def __buildTableList(self, showFieldList):
        ...
        if len(showFieldList):
            ...
            if tableList.count("Sites") and tableList.count("MessageRepository") and not tableList.count("ClientIPs"):
                tableList.append("ClientIPs")
            if (
                tableList.count("MessageRepository")
                and tableList.count("SubSystems")
            ...
            if (
                tableList.count("MessageRepository")
                and tableList.count("Systems")
                and not tableList.count("FixedTextMessages")
            ):
                tableList.append("FixedTextMessages")
            if (
                tableList.count("FixedTextMessages")
                and tableList.count("SubSystems")
                and not tableList.count("Systems")
            ):
                tableList.append("Systems")
            if tableList.count("MessageRepository") or (
                tableList.count("FixedTextMessages") + tableList.count("ClientIPs") + tableList.count("UserDNs") > 1
            ):
                tableString = "MessageRepository"
    ...
    def _queryDB(
        self, showFieldList=None, condDict=None, older=None, newer=None, count=False, groupColumn=None, orderFields=None
    ):
        ...
        cmd = "SELECT %s FROM %s %s %s %s" % (",".join(showFieldList), tableList, condition, grouping, ordering)
