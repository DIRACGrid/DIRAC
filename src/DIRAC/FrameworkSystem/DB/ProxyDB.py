...
class ProxyDB(DB):
  ...
  def _isProxyExist(self, userDN, timeleft=None):
    """ Check if proxy present in DB

        :param str userDN: user DN
        :param int timeleft: requirement time
    ...
    if not result['OK']:
      return result
    sUserDN = result['Value']
    cmd = 'SELECT * FROM ProxyDB_CleanProxies WHERE UserDN = %s' % sUserDN
    if timeleft:
      cmd += ' AND TIMESTAMPDIFF(SECOND, UTC_TIMESTAMP(), ExpirationTime) > %s' % timeleft
    result = self._query(cmd)
    return S_OK(True if result['Value'] else False) if result['OK'] else result
  ...
  @deprecated("Old method for compatibility with older versions v7r0-")
  ...
  def getUsers(self, validSecondsLeft=0, userMask=None):
    ...
    result = self.getProxiesContent(selDict, cond=sqlCond)
  ...
  def getProxiesContent(self, selDict, sortList=None, start=0, limit=0, cond=None):
    """ Get the contents of the db, parameters are a filter to the db.
        ...
        :param sortList: dict with sorting fields
        :type sortList: str or list
        :param int start: search limit start
        :param int limit: search limit amount
        :param dict cond: filters
    ...
    if cond:
      sqlWhere += (list(cond) if isinstance(cond, (list, tuple)) else [cond])
    ...
    if sortList:
      for sort in sortList:
        ...
        if sort[0] in ['UserDN', 'ExpirationTime']:
    ...
    for table, fields in [('ProxyDB_CleanProxies', ("UserDN", "ExpirationTime")),
      ...
      cmd += " ORDER BY %s" % (", ".join(sqlOrder) if sqlOrder else "UserDN DESC")
      retVal = self._query(cmd)
      if not retVal['OK']:
        retVal['Message'] += "\n" + cmd
        return retVal
