...
class SystemAdministratorIntegrator:
    def __init__(self, **kwargs):
        ...
        for host in self.__hosts:
            pool.generateJobAndQueueIt(
                self.__executeClient, args=[host, "ping"], kwargs={}, oCallback=self.__processResult
            )
    ...
    def execute(self, *args, **kwargs):
        ...
        self.__resultDict = {}
        for host in self.__hosts:
            self.__pool.generateJobAndQueueIt(
                self.__executeClient, args=[host, self.call] + list(args), kwargs=kwargs, oCallback=self.__processResult
            )
