    ...
    def _getCentralCfg(self, installCfg):
        ...
        if adminUserName:
            if not (adminUserDN and adminUserEmail):
                ...
                if centralCfg["Registry"]["Users"][adminUserName].existsKey(
                    "DN"
                ):  # pylint: disable=unsubscriptable-object
                    centralCfg["Registry"]["Users"][adminUserName].deleteKey(
                        "DN"
                    )  # pylint: disable=unsubscriptable-object
                ...
                for group in [adminGroupName, defaultGroupName]:
                    ...
                    if adminUserName not in users:
                        centralCfg["Registry"]["Groups"][
                            group
                        ].appendToOption(  # pylint: disable=unsubscriptable-object
                ...
                properties = centralCfg["Registry"]["Groups"][
                    adminGroupName
                ].getOption(  # noqa # pylint: disable=unsubscriptable-object
                    "Properties", []
                )
                    ...
                    for component in softDict[compType][system]:
                        ...
                        try:
                            ...
                            if compDir in runitDict:
                                resultDict[compType][system][component]["RunitStatus"] = runitDict[compDir][
                                    "RunitStatus"
                                ]
  ...
  """
    ...
    def installComponent(self, componentType, system, component, extensions, componentModule="", checkModule=True):
        ...
        if checkModule:
            ...
            if not result["OK"]:
                if (
                    not self.checkComponentSoftware(componentType, system, cModule, extensions)["OK"]
...
exec dirac-webapp-run -p < /dev/null
  """
    ...
    def getAvailableSQLDatabases(self, extensions):
        ...
        for extension in extensions + [""]:
            databases = glob.glob(
                os.path.join(rootPath, ("%sDIRAC" % extension).replace("DIRACDIRAC", "DIRAC"), "*", "DB", "*.sql")
            )
    ...
    def getAvailableESDatabases(self, extensions):
        ...
        for extension in extensions + [""]:
            ...
            pyDBs = glob.glob(
                os.path.join(rootPath, ("%sDIRAC" % extension).replace("DIRACDIRAC", "DIRAC"), "*", "DB", "*DB.py")
            )
    ...
    def installDatabase(self, dbName):
        ...
        for extension in CSGlobals.getCSExtensions():
            dbFileInExtension = glob.glob(os.path.join(rootPath, "%sDIRAC" % extension, "*", "DB", "%s.sql" % dbName))
        ...
        for cmd in [
            "GRANT %s ON `%s`.* TO '%s'@'localhost'" % (perms, dbName, self.mysqlUser),
