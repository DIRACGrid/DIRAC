  ...
  def validateIdentityProvider(self, request, provider):
    ...
    if request.provider:
      if request.provider not in idPs:
        ...
        raise Exception('The %s group Identity Provider is "%s" and not "%s".' % (request.group, groupProvider,
                                                                                  request.provider))
      return request
    ...
    # Render into HTML
    return getHTML("Identity Provider selection..", body=tag, icon='fingerprint',
