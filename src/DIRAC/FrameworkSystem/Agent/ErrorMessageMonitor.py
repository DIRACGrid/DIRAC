...
class ErrorMessageMonitor(AgentModule):
    ...
    def execute(self):
        ...
        result = self.systemLoggingDB._queryDB(
            showFieldList=returnFields, groupColumn="FixedTextString,FixedTextID,SystemName", condDict=condDict
        )
        if not result["OK"]:
            self.log.error("Failed to obtain the non reviewed Strings", result["Message"])
            return S_OK()
        ...
        self.log.info("The messages have been sent for review", "There are %s new descriptions" % len(messageList))
