...
def tdFixture():
    ...
    taskDicts = [
        dict(TaskID=123, LFN="lfn123", Status="Assigned", FileID=987001, ErrorCount=9),
...
def test_findAllDescendants(tiFixture):
    ...
    tiFixture.fcClient.getFileDescendents = Mock(
        return_value=S_OK(
            {
                "Successful": {
                    "lfn1": ["lfnD1", "lfnD2"],
...
def test_cleanOutputs(tiFixture, userProxyFixture):
    ...
    tiFixture._TransformationInfo__findAllDescendants = Mock(return_value=descList)
    with patch(
        "DIRAC.TransformationSystem.Utilities.TransformationInfo.DataManager", return_value=remMock, autospec=True
    ):
        with pytest.raises(RuntimeError) as re:
            tiFixture.cleanOutputs(jobInfo)
    ...
    remMock = Mock(name="remmock")
    remMock.removeFile.return_value = S_OK(
        {
            "Successful": {"lfn1": "OK", "lfn2": "OK"},
            "Failed": {"lfnD2": "SomeReason", "lfnD3": "SomeReason", "lfnDD2": "SomeOtherReason"},
        }
    )

    tiFixture.enabled = True
    with patch(
        "DIRAC.TransformationSystem.Utilities.TransformationInfo.DataManager", autospec=True, return_value=remMock
    ):
        tiFixture.cleanOutputs(jobInfo)
