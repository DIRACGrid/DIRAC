...
class PluginUtilities(object):
    ...
    def __init__(
        ...
        self.log = gLogger.getSubLogger(
            self.plugin + self.transInThread.get(self.transID, " [NoThread] [%s] " % self.transID)
        )
    ...
    def logVerbose(self, message, param=""):
        ...
        if self.debug:
            log = gLogger.getSubLogger(
                self.plugin + " (V)" + self.transInThread.get(self.transID, " [NoThread] [%d] " % self.transID)
            )
    ...
    def setParameters(self, params):
        ...
        self.log = gLogger.getSubLogger(
            self.plugin + self.transInThread.get(self.transID, " [NoThread] [%d] " % self.transID)
        )
