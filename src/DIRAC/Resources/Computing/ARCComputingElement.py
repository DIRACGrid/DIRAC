        ...
        if self.rest:
            ...
            for __i in range(numberOfJobs):
                ...
                if 199 <= int(r.status_code) < 250:
                    ...
                    self.log.warn(
                        "Failed to submit job to CE %s with REST (https) interface error %s."
                        % (self.ceHost, r.status_code)
                    )
            ...
            for __i in range(numberOfJobs):
                ...
                if result == arc.SubmissionStatus.NONE:
                    ...
                    if result.isSet(arc.SubmissionStatus.NOT_IMPLEMENTED):  # pylint: disable=no-member
                        self.log.warn(
                            "%s feature not implemented on CE? (weird I know - complain to site admins" % message
                        )
        ...
        for job in jobList:
            ...
            if int(r.status_code) == 201:  # Job can be renewed
                ...
                if "delegation_id" in p["job"]:
                    ...
                    with open(tmpProxyFile, "w") as outFile:
