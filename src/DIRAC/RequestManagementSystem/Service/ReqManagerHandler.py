"""Implementation of the ReqManager service
...
class ReqManagerHandlerMixin:
    ...
    def export_putRequest(self, requestJSON):
        ...
        gLogger.info("putRequest: Attempting to set request", requestName)
    ...
    def export_getScheduledRequest(cls, operationID):
        ...
        if not scheduled["OK"]:
            gLogger.error("getScheduledRequest", scheduled["Message"])
            return scheduled
        if not scheduled["Value"]:
            return S_OK()
        requestJSON = scheduled["Value"].toJSON()
        if not requestJSON["OK"]:
            gLogger.error("getScheduledRequest", requestJSON["Message"])
        return requestJSON
    ...
    def export_getRequest(cls, requestID=0):
        ...
        if not getRequest["OK"]:
            gLogger.error("getRequest", getRequest["Message"])
            return getRequest
        if getRequest["Value"]:
    ...
    def export_getBulkRequests(cls, numberOfRequest, assigned):
        ...
        if not getRequests["OK"]:
            gLogger.error("getRequests", getRequests["Message"])
            return getRequests
    ...
    def export_peekRequest(cls, requestID=0):
        ...
        if not peekRequest["OK"]:
            gLogger.error("peekRequest", peekRequest["Message"])
            return peekRequest
    ...
    def export_getRequestIDsList(cls, statusList=None, limit=None, since=None, until=None, getJobID=False):
        ...
        if not reqIDsList["OK"]:
            gLogger.error("getRequestIDsList", reqIDsList["Message"])
        return reqIDsList
    ...
    def export_readRequestsForJobs(cls, jobIDs):
        ...
        if not requests["OK"]:
            gLogger.error("readRequestsForJobs", requests["Message"])
            return requests
    ...
    def export_getRequestFileStatus(cls, requestID, lfnList):
        ...
        if not res["OK"]:
            gLogger.error("getRequestFileStatus", res["Message"])
        return res
    ...
    def export_getRequestInfo(cls, requestID):
        ...
        if not requestInfo["OK"]:
            gLogger.error("getRequestInfo", requestInfo["Message"])
        return requestInfo
