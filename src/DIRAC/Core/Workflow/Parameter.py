...
class Parameter:
    ...
    def __str__(self):
        return (
            ...
            + self.name
            + " value="
            + str(self.getValueTypeCorrected())
            + " type="
            + str(self.type)
            + " linked_module="
            + str(self.linked_module)
            + " linked_parameter="
            + str(self.linked_parameter)
            + " in="
            + str(self.typein)
            + " out="
            + str(self.typeout)
    ...
    def compare(self, s):
        if isinstance(s, Parameter):
            return (
                (self.name == s.name)
                and (self.value == s.value)
                and (self.type == s.type)
                and (self.linked_module == s.linked_module)
                and (self.linked_parameter == s.linked_parameter)
                and (self.typein == s.typein)
...
class ParameterCollection(list):
    ...
    def setValue(self, name, value, vtype=None):
        ...
        if par is None:
            print(
                "ERROR ParameterCollection.setValue() can not find parameter with "
    ...
    def linkUp(self, opt, prefix="", postfix="", objname="self"):
        ...
        if isinstance(opt, ParameterCollection):
            ...
            for p in opt:
                ...
                if par is None:
                    print(
                        "WARNING ParameterCollection.linkUp can not find parameter with the name=",
                        p.getName(),
                        " IGNORING",
                    )
                else:
