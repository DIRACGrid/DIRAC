...
class GraphData:
    ...
    def getPlotNumData(self, label=None, zipFlag=True):
        ...
        if self.plotdata:
            ...
            for label in self.subplots:
                arrays.append(
                    numpy.array([x[1] for x in self.subplots[label].getPlotDataForNumKeys(self.all_num_keys, True)])
                )
        ...
        for num_key in num_keys:
            try:
                ...
                if self.values[ind] is None and zeroes:
                    result_pairs.append((self.num_keys[ind], 0.0, 0.0))
                else:
                    result_pairs.append((self.num_keys[ind], self.values[ind], self.errors[ind]))
            except ValueError:
                if zeroes:
                    result_pairs.append((num_key, 0.0, 0.0))
                else:
                    result_pairs.append((num_key, None, 0.0))
