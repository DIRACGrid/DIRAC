#!/usr/bin/env python
########################################################################
# $Header: /tmp/libdirac/tmp.stZoy15380/dirac/DIRAC3/DIRAC/FrameworkSystem/scripts/Attic/dirac-admin-get-pilot-proxy.py,v 1.1 2008/07/23 13:37:10 acasajus Exp $
# File :   dirac-admin-get-proxy
# Author : Stuart Paterson
########################################################################
__RCSID__   = "$Id: dirac-admin-get-pilot-proxy.py,v 1.1 2008/07/23 13:37:10 acasajus Exp $"
...
if not params.proxyPath:
  ...
  params.proxyPath = "%s/proxy.pilot.%s.%s" % ( os.getcwd(), userName, userGroup )


result = gProxyManager.getPilotProxyFromDIRACGroup( userDN, userGroup, requiredTimeLeft = params.proxyLifeTime )
if not result['OK']:
