...
class OAuthHandler( RequestHandler ):
    ...
    for DN in result['Value']:
      if group in Registry.getGroupsFromDNProperties( DN ):
        if voms:
          ...
          result = gProxyManager.downloadVOMSProxy( DN, group, requiredVOMSAttribute = voms, requiredTimeLeft=proxylivetime )
        else:
  ...
  def export_waitStateResponse(self, state, group=None, needProxy=False, voms=None, proxyLifeTime=43200, time_out=20, sleeptime=5):
