...
class AuthHandler(WebHandler):
  ...
  def web_auth(self):
    """ REST endpoint for users authentication.
        ...
        **GET** /auth/<IdP>?<options> -- this request for initialization authetication flow  
          * *IdP* -- is a registred in DIRAC Identity provider name that need to use for authentication
          
          Options:
            * *email* -- send the authentcation URL to the email(optional)
          
          Responce in the json format with the next keys:
            * *Status* -- session status, required values: "needToAuth", "ready", "fail".
            * *URL* -- link for authetication, generated by choosed identity provider. Returned if status is "needToAuth".
        ...
        **GET** /auth/<session> -- redirect to identity provider authentication endpoint to enable the user to authenticate  
          * *session* -- session ID
          ...
          Responce depended from redirected endpoint
