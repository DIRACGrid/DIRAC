...
class ProxyDB(DB):
  ...
  def __init__(self,
    ...
    self.__oauthClient = OAuthClient()
  ...
  def storeProxy(self, userDN, userGroup, chain, proxyProvider=None):
    ...
    if sqlInsert:
      ...
      sqlWhere = []
      for k in dValues:
        if k in ('UserDN', 'UserGroup', 'ProxyProvider'):
  ...
  def __getDIRACCAProxy(self, fullName, eMail, DN=None, diracGroup=None, proxyProvider='DIRAC_EOSC_CA'):
    ...
    with open(caDir+'/index.txt', 'w') as ind:
      ind.write('')
    status,output = commands.getstatusoutput('openssl ca -config %s -extensions usr_cert -batch -days 375 -in %s -out %s' % \
                                            (caConfigFile, userReqFile, userCertFile) )
    if status:
      return S_ERROR(output)                                 
    chain = X509Chain()
