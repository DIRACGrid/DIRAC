...
class OAuthDB(DB):
  ...
  def get_link_by_state( self, state ):
    ...
    result = self._getFromWhere('Comment', 'Tokens', conn='Status = "prepared" and TIMESTAMPDIFF(SECOND,LastAccess,UTC_TIMESTAMP()) < 300', State=state)
    if not result[ 'OK' ]:
      return result
  ...
  def parse_auth_response( self, code, state ):
    ...
    if OAuthProvider in Resources.getProxyProviders()['Value']:
      ...
      status = 'authed'
      self.updateFields( 'Tokens', [ 'Status','Token_type','Access_token','Expires_in','Refresh_token','Sub','UserName','LastAccess' ], 
                                   [ status,oauthDict['Tokens']['token_type'],oauthDict['Tokens']['access_token'],exp_datetime,
                                   oauthDict['Tokens']['refresh_token'],oauthDict['UserProfile']['sub'],csModDict['username'],'UTC_TIMESTAMP()' ],
                                   {'State': state}) 
      ...
      csModDict['UsrOptns']['DNProperties/%s/Groups' % proxyDN.replace('/','-').replace('=','_')] = ','.join(csModDict['UsrOptns']['Groups'])
      csModDict['UsrOptns']['DNProperties/%s/ProxyProviders' % proxyDN.replace('/','-').replace('=','_')] = OAuthProvider
      ...
      if not csModDict['UsrOptns']['Groups'] and not csModDict['Groups'] and not csModDict['UsrExist']:
        comment = 'We not found any registred DIRAC groups that mached with your profile. So, your profile has the same access that Visitor DIRAC user.'
        ...
        if pP:
          ...
          if method == 'oAuth2':
            ...
            if result['OK']:
              ...
              if 'Groups' not in csModDict['UsrOptns']:
                ...
                return S_ERROR( 'Cannot found any groups in IdP record field' )
              csModDict['UsrOptns']['DNProperties/%s/Groups' % proxyDN.replace('/','-').replace('=','_')] = ','.join(csModDict['UsrOptns']['Groups'])
              csModDict['UsrOptns']['DNProperties/%s/ProxyProviders' % proxyDN.replace('/','-').replace('=','_')] = pP
          ...
          _CRASH('No DN returned from %s OAuth provider' % OAuthProvider)
          return S_ERROR( 'No DN returned from %s OAuth provider' % OAuthProvider )
      self.updateFields( 'Tokens', [ 'Status','Comment','Token_type','Access_token','Expires_in','Refresh_token','Sub','UserName','LastAccess' ], 
                                   [ status,comment,oauthDict['Tokens']['token_type'],oauthDict['Tokens']['access_token'],exp_datetime,
                                     oauthDict['Tokens']['refresh_token'],oauthDict['UserProfile']['sub'],oauthDict['username'],'UTC_TIMESTAMP()' ],
                                   {'State': state})
    else:
    ...
    if not oauthDict['redirect']:
      ...
      if 'noregvos' in csModDict:
        oauthDict['messages'].append('We found not registred VOs in your profile: %s. Please contact with administrators of this VOs to register it in DIRAC.' % str(csModDict['noregvos']))
  ...
  def get_by_state( self, state, value=['OAuthProvider','Sub','State','Status','Comment','Token_type','Access_token','Expires_in','Refresh_token','UserName','LastAccess'] ):
  ...
  def fetch_token( self, token=None, state=None ):
    ...
    result = self._getListFromWhere(['Access_token','Expires_in','Refresh_token','OAuthProvider'], 'Tokens', conn='Status = "authed"', **params)
    if not result['OK']:
      return result
    resD = result['Value']
