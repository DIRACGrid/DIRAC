#!/usr/bin/env python
# $Header: /tmp/libdirac/tmp.stZoy15380/dirac/DIRAC3/DIRAC/AccountingSystem/scripts/Attic/dirac-accounting-admin-cli.py,v 1.1 2008/03/05 21:02:27 acasajus Exp $
__RCSID__ = "$Id: dirac-accounting-admin-cli.py,v 1.1 2008/03/05 21:02:27 acasajus Exp $"
...
class AccountingCLI( cmd.Cmd ):
  ...
  def do_showRegisteredTypes( self, args ):
    ...
    """
    try:
      retVal = self.acClient.getRegisteredTypes()
      if not retVal[ 'OK' ]:
        gLogger.error( "Error: %s" % retVal[ 'Message' ] )
        return
  ...
  def do_showRegisteredKeyFields( self, args ):
    ...
    """
    try:
      retVal = self.acClient.getRegisteredKeyFields()
      if not retVal[ 'OK' ]:
        gLogger.error( "Error: %s" % retVal[ 'Message' ] )
        return
    ...
    try:
      ...
      while True:
        ...
        if choice in ( "yes", "y" ):
          ...
          return
      retVal = self.acClient.deleteType( typeName )
      if not retVal[ 'OK' ]:
        gLogger.error( "Error: %s" % retVal[ 'Message' ] )
        return
