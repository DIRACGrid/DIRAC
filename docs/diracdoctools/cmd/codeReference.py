    ...
    def getmodules(self, abspath, _direc, files):
        ...
        for filename in files:
            if (
                ...
                or filename.lower().endswith("test")
                or any(f.lower() in filename.lower() for f in self.config.code_ignoreFiles)
            ):
        ...
        for root, direc, files in os.walk(self.config.sourcePath):
            ...
            if firstModule:
                ...
                self.createCodeDocIndex(
                    subpackages=packages, modules=self.getmodules(codePath, direc, files), buildtype=buildtype
                )
                ...
                self.mkPackageRst(
                    ...
                    modules=self.getmodules(docPath, direc, files),
                )
            ...
            for filename in files:
                ...
                if any(f in filename for f in self.config.code_dummyFiles):
                    ...
                    or filename.endswith("CLI.py")
                    or filename.lower().startswith("test")
                    or filename == "__init__.py"
                    or any(f in filename for f in self.config.code_ignoreFiles)
                    or "-" in filename
                ):  # not valid python identifier, e.g. dirac-pilot
