...
Creating a TransformationAgent Plugin
=====================================

This section needs to be done with the ``dirac`` user to modify the ``Server`` code of the transformation system located

We now want to create a plugin, which gives us control over which files the transformation system sends around.

The file ``/opt/dirac/pro/DIRAC/TransformationSystem/Agent/TransformationPlugin.py`` contains the
``TransformationPlugins`` for DIRAC.

We will create a plugin 'EvenOdd', that sorts the files in even an odd numbers and creates tasks in groups of Even or
Odd files, or just ignores Even or Odd files entirely.

Add New Plugin to the List of Allowed Plugins
---------------------------------------------

In the ``Configuration Manager`` of the ``WebApp`` add the section ``Operations/Defaults/Transformations`` with the
parameter::

  Transformations
  {
    AllowedPlugins = Broadcast, Standard, BySize, ByShare, EvenOdd
  }

The first four entries are the default allowed plugins

The EvenOdd Plugin
------------------

Add this function to the end of ``DIRAC/TransformationSystem/Agent/TransformationPlugin.py``.

.. code-block:: python
   :caption: DIRAC/TransformationSystem/Agent/TransformationPlugin.py
   :linenos:
    ...
    def _EvenOdd(self):
      ...
      :returns: S_OK with list of tuples. Each Tuple is a pair of SE and list of LFNs to treat in given task
      """
      # log parameters
...
After adding the ``AllowedPlugins`` option and the code for the ``EvenOdd`` plugin, we should restart the
TransformationAgent, so it reloads the source code.
...
This section needs to be executed as the ``diracuser`` with a ``dirac_prod`` proxy.
...
Then execute the script and follow the output of the TransformationAgent to see the Plugin in action::

  [diracuser@dirac-tuto ~]$ python createEvenOdd.py
  Created transformation AAA
  Added input data query {'OK': True, 'rpcStub': (('Transformation/TransformationManager', {'skipCACheck': False, 'keepAliveLapse': 150, 'timeout': 120}), 'createTransformationInputDataQuery', (AAAL, {'TransformationID': 2})), 'Value': 1L}
  Created RemoveReplica transformation: AAAL
  [diracuser@dirac-tuto ~]$ less +F --follow-name /opt/dirac/pro/runit/Transformation/TransformationAgent/log/current
...
You now have all the knowledge to perform DataManagement in DIRAC with the TransformationSystem, including how to extend
