 ...
 * `StageProtocols`: used when issuing a stage request (see bellow)
...
Up to recently, any protocol that was defined as `AccessProtocol` was also used for staging (plot twister: only `srm` could do it).
