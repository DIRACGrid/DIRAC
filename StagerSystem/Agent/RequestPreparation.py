# $Header: /tmp/libdirac/tmp.stZoy15380/dirac/DIRAC3/DIRAC/StagerSystem/Agent/RequestPreparation.py,v 1.1 2009/06/15 18:39:51 acsmith Exp $

"""  TransferAgent takes transfer requests from the RequestDB and replicates them
"""

__RCSID__ = "$Id: RequestPreparation.py,v 1.1 2009/06/15 18:39:51 acsmith Exp $"
...
class RequestPreparation(Agent):
    ...
    for lfn,metadata in fileMetadata.items():
      ...
      for storageElement in lfnToSE[lfn].keys():
        if not replicas.has_key(storageElement):
          gLogger.error(": File does not have replica at requested storage element","%s %s" % (lfn,storageElement))
  ...
  def __clearFailedFileTasks(self,failedFileIDs):
    ...
    # Then get all the files associated to that task
  ...
  def __reportProblematicFiles(self,lfns,reason):
    res = self.dataIntegrityClient.setFileProblematic(self,missing,reason,self.name)
    if not res['OK']:
  ...
  def __getFileMetadata(self,lfns):
    ...
    terminal.extend(res['Value']['ZeroSize'])
    ...
    if not res['OK']:
      return res
    failed.update(res['Value']['Failed'])
    terminal.extend(res['Value']['ZeroReplicas'])
  ...
  def __getExistingFiles(self,lfns):
    ...
    if missing:
      for lfn in missing:
        gLogger.warn("RequestPreparation.__getExistingFiles: Requested file did not exist",lfn)
  ...
  def __getFileSize(self,lfns):
    ...
    if zeroSize:
      for lfn in zeroSize:
        gLogger.warn("RequestPreparation.__getFileSize: Requested file has zero size",lfn)
      self.__reportProblematicFiles(noReplicas,'LFN-LFC-ZeroSize')
  ...
  def __getFileReplicas(self,lfns):
    ...
    if noReplicas:
      for lfn in noReplicas:
        gLogger.warn("RequestPreparation.__getFileReplicas: Requested file has no replicas",lfn)
      self.__reportProblematicFiles(noReplicas,'LFN-LFC-NoReplicas')
