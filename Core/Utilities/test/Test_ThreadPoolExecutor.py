...
def tCallback( timeWait, futures ):
...
class ThreadPoolExecutorTests( unittest.TestCase ):
  ...
  def test_generateJobAndQueueItI( self ):  
    for _ in xrange( 20 ):
      ...
      self.threadPool.generateJobAndQueueIt( tFunc, args = ( timeWait, ) )
  ...
  def test_generateJobAndQueueItICallback( self ):
    for i in xrange( 20 ):
      ...
      self.threadPool.generateJobAndQueueIt( tFunc, args = ( timeWait, ) )
      if i % 2:
        self.threadPool.generateJobAndQueueIt( tFunc, args = ( timeWait, ), oCallback = functools.partial( tCallback, "Future:%s" % timeWait ) ) 
      else:
        self.threadPool.generateJobAndQueueIt( tFunc, args = ( timeWait, ) )
