...
class RequestTask( object ):
  ...
  def changeProxy( self, ownerDN, ownerGroup ):
    ...
    # Include the requestname in the proxy filename in order to make it really unique...
    fd, ownerProxyFile = tempfile.mkstemp( suffix = '.' + self.requestName )
    os.close( fd )
    ownerProxyFile = ownerProxy["Value"].dumpAllToFile( ownerProxyFile )
  ...
  def __call__( self ):
    ...
    if ownerDN and ownerGroup:
      ...
      if not ownerProxyFile["OK"]:
        self.error( "handleRequest: unable to get proxy for '%s'@'%s': %s" % ( ownerDN,
                                                                               ownerGroup,
    ...
    for index in range( res["Value"] ):
      ...
      if subExecutionOrder <= self.executionOrder:
        ...
        if operation not in self.operationDispatcher():
          ...
          self.info( "handleRequest: will execute '%s' subrequest of order %s" % ( operation, str( index ) ) )
