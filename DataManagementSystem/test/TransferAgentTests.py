...
class TransferTaskTests( unittest.TestCase ):
  ...
  def test_02_putAndRegister( self ):
    ...
    self.assertEqual( tTask.__call__(), { 'OK' : True, 'Value' : { 'monitor' : { 'File registration successful': 1, 
                                                                                 'Execute': 1, 
                                                                                 'Done': 1, 
                                                                                 'Put successful': 1, 
                                                                                 'Put and register': 1 } } } )
  ...
  def test_05_replicateAndRegister( self ):
    ...
    self.assertEqual( tTask(), {'OK': True, 'Value': {'monitor': {'Replica registration successful': 1, 'Execute': 1, 'Done': 1, 'Replication successful': 1, 'Replicate and register': 1}}} )
    del tTask
...
class TransferAgentTests( unittest.TestCase ):
  ...
  def test__01_ctor( self ):
    ...
    tAgent = None
    try:
      tAgent = TransferAgent( AGENT_NAME )
    except:
      pass
    self.assertEqual( isinstance( tAgent, TransferAgent ), True )
    del tAgent

  def test_02_schedule( self ):
    tAgent = TransferAgent( AGENT_NAME )
    ## sanitize getRequest
    tAgent.getRequest = Mock( return_value = getRequest( "replicateAndRegister") )
    ## sanitise TransferDB
    tAgent.transferDB = Mock( return_value = Mock( spec = TransferDB ) )
    tAgent.transferDB().getChannelQueues = Mock()
    tAgent.transferDB().getChannelQueues.return_value = getChannelQueues()
    tAgent.transferDB().getChannelObservedThroughput = Mock()
    tAgent.transferDB().getChannelObservedThroughput.return_value = getChannelObservedThroughput()
    tAgent.transferDB().addFileToChannel = Mock( return_value = SOK )
    tAgent.transferDB().addFileRegistration = Mock( return_value = SOK )
    tAgent.transferDB().addReplicationTree = Mock( return_value = SOK )
    ## sanitize RequestDBMySQL
    tAgent.requestDBMySQL = Mock( return_value = Mock(spec = RequestDBMySQL ) )
    tAgent.requestDBMySQL().updateRequest = Mock( return_value = SOK )

    ## sanitise ReplicaManager
    tAgent.replicaManager = Mock( return_value = Mock( spec = ReplicaManager ) )
    tAgent.replicaManager().getCatalogReplicas = Mock()
    tAgent.replicaManager().getCatalogReplicas.return_value = getCatalogReplicas()    
    tAgent.replicaManager().getCatalogFileMetadata = Mock()
    tAgent.replicaManager().getCatalogFileMetadata.return_value = getCatalogFileMetadata()
    ## sanitise RequestClient
    tAgent.requestClient = Mock( return_value = Mock(spec=RequestClient) ) 
    tAgent.requestClient().updateRequest = Mock()
    tAgent.requestClient().updateRequest.return_value = SOK
    tAgent.requestClient().finalizeRequest = Mock()
    tAgent.requestClient().finalizeRequest.return_value = SOK
    ## sanitize ProcessPool
    tAgent.processPool = Mock( return_value = Mock( spec = ProcessPool) )
    tAgent.processPool().createAndQueueTask = Mock( return_value = SOK )
    tAgent.processPool().processResults = Mock( return_value = SOK )

    
    ret = tAgent.initialize()
    print ret
    ret = tAgent.setupStrategyHandler()
    print ret

    ret = tAgent.schedule( getRequest("replicateAndRegister")["Value"] )
    print ret
