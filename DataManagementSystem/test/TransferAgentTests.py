...
class TransferAgentTests( unittest.TestCase ):
  ...
  def test_02_schedule( self ):
    ...
    ## execute 
    self.assertEqual( self.tAgent.setupStrategyHandler(), {'OK': True, 'Value': ''} )
    self.assertEqual( self.tAgent.schedule( getRequest("replicateAndRegister")["Value"] ), {'OK': True, 'Value': ''} )

## test suite exucution
if __name__ == "__main__":
  ...
  ## a = [ (1,2,3), (1,2,4)]
  ## keys = [ "one", "two", "three" ]
  ## [ dict(zip(keys, values)) for values in a ]
