  ...
  def __call__( self ):
    ...
    for opFile in waitingFiles:
      ...
      if not registerFile["OK"] or lfn in registerFile["Value"]["Failed"]:
        ...
        self.dataLoggingClient().addFileRecord( lfn, "RegisterFail", ','.join( catalogs ) if catalogs else "", "", "RegisterFile" )
        ...
        self.dataLoggingClient().addFileRecord( lfn, "Register", ','.join( catalogs ) if catalogs else "", "", "RegisterFile" )

        self.log.info( "file %s has been registered at %s" % ( lfn, ','.join( catalogs ) if catalogs else "" ) )
        opFile.Status = "Done"
