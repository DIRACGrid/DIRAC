...
def listDirectory(n_queries):
    ...
    if verbosity >= 1:
        print(
            "getReplicas: Total time", total, "Success", sCount, "Failure", fCount, "Average", average, "Stdvar", error
        )
...
def getDirectoryReplicas(n_queries):
    ...
    if verbosity >= 1:
        print(
            "getDirectoryReplicas: Total time",
            total,
            "Success",
            sCount,
            "Failure",
...
directoriesList = [
    (35455, "/auger/prod/QGSjetII_gr20_simADSTv2r5p1/en18.000/th0.65/2008/11/12"),
