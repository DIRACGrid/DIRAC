...
def createWorkflowBodyStep1():
  ...
  outputSE = 'DIRAC-USER'
  outputMetadata = json.dumps( {"application":"mandelbrot","image_format":"ascii", "image_width":7680, "image_height":200} )
...
def createWorkflowBodyStep2():
  ...
  outputSE = 'DIRAC-USER'
  nb_input_files = 7
  outputMetadata = json.dumps( {"application":"mandelbrot","image_format":"ascii", "image_width":7680, "image_height":200*nb_input_files} )
  job.setExecutable( './mandel4ts/dirac-add-files.py', arguments = "%s '%s' %s '%s'" % (outputPath, outputPattern, outputSE, outputMetadata ) )
  return job.workflow.toXML()
...
fc = FileCatalog()
MDFieldDict = {'application': 'VARCHAR(128)', 'image_format': 'VARCHAR(128)', 'image_width': 'int', 'image_height': 'int'}
