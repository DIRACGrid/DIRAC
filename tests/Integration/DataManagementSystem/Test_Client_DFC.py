...
class UserGroupCase(DFCTestCase):
    def test_userOperations(self):
        ...
        if isAdmin:
            ...
            self.assertTrue(testUser in result["Value"], f"getUsers failed: {result}")
...
class FileCase(DFCTestCase):
    def test_fileOperations(self):
        ...
        self.assertTrue(
            ...
            f"addFile failed: it should  be possible to add an existing lfn with the same attributes {result}",
        )
        ...
        self.assertTrue(result["OK"], f"addFile failed when adding non existing file with existing GUID {result}")
        self.assertTrue(
            testFile + "2" in result["Value"]["Failed"],
            f"addFile failed: it should not be possible to add an existing GUID {result}",
        )
        ...
        if isAdmin:
            self.assertEqual(
                ...
                "toto",
                f"getFileMetadata got incorrect Owner {result}",
            )
