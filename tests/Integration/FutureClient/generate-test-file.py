...
def main(system, component):
    ...
    old_client = getattr(importlib.import_module(f"DIRAC.{system}System.Client.{client_name}"), client_name)
    future_client = getattr(importlib.import_module(f"DIRAC.{system}System.FutureClient.{client_name}"), client_name)
