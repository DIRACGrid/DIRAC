...
class ProxyDBTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(cls.caConfigFile) as caCFG:
            for line in caCFG:
                if re.findall("=", re.sub(r"#.*", "", line)):
                    ...
                    line = f"dir = {cls.caPath} #PUT THE RIGHT DIR HERE!\n" if field == "dir" else line
                    val = re.sub(r"#.*", "", line).replace(" ", "").rstrip().split("=")[1]
                    if field in fields:
                        for i in fields:
                            if cfgDict.get(i):
                                val = val.replace(f"${i}", cfgDict[i])
                        cfgDict[field] = val
        ...
        """ % (
            ...
            with open(userConfFile, "w") as f:
                f.write(userConf)
            status, output = commands.getstatusoutput(f"openssl genrsa -out {userKeyFile}")
            if status:
        ...
        # Result
        status, output = commands.getstatusoutput(f"ls -al {cls.userDir}")
        if status:
            gLogger.error(output)
            exit()
    ...
    def tearDownClass(cls):
        ...
        for f in os.listdir(cls.caPath):
            if re.match(f"{cls.index}..*", f) or f.endswith(".old"):
...
class testDB(ProxyDBTestCase):
    ...
    def test_getUsers(self):
        ...
        for table, values, fields in [
            ...
            result = db._update(f"INSERT INTO {table}{fields} VALUES {', '.join(values)} ;")
    ...
    def test_purgeExpiredProxies(self):
        ...
        self.assertTrue(result["OK"], "\n" + result.get("Message", "Error message is absent."))
        cmd = "SELECT COUNT( * ) FROM ProxyDB_CleanProxies WHERE ExpirationTime < UTC_TIMESTAMP()"
    ...
    def test_getRemoveProxy(self):
        ...
        for dn, group, reqtime, log in [
            (
                ...
                "group_1",
                9999,
                "No proxy provider, set request time, not valid proxy in ProxyDB_CleanProxies",
            ),
            ("/C=CC/O=DN/O=DIRAC/CN=user", "group_1", 0, "Not valid proxy in ProxyDB_CleanProxies"),
        ...
        cmd = 'SELECT COUNT( * ) FROM ProxyDB_CleanProxies WHERE UserName="user_ca"'
        self.assertTrue(
            bool(db._query(cmd)["Value"][0][0] == 1),
            "ProxyDB_CleanProxies" + " must " + (1 and "contain proxy" or "be empty"),
        )
        ...
        for user, dn, group, vo, time, res, log in [
            ...
            result = db._update('DELETE FROM ProxyDB_CleanProxies WHERE UserName = "user"')
            self.assertTrue(result["OK"], "\n" + result.get("Message", "Error message is absent."))
            result = db._update('DELETE FROM ProxyDB_CleanProxies WHERE UserName = "user_1"')
            self.assertTrue(result["OK"], "\n" + result.get("Message", "Error message is absent."))
            self.assertTrue(result["OK"], "\n" + result.get("Message", "Error message is absent."))
            gLogger.info(f"== > {log}:")
            ...
            if not res:
                gLogger.info(f"Msg: {result['Message']}")
            cmd = f'SELECT COUNT( * ) FROM ProxyDB_CleanProxies WHERE UserName="{user}"'
            self.assertTrue(
        ...
        for table, user, count in [("ProxyDB_CleanProxies", "user", 1), ("ProxyDB_CleanProxies", "user_ca", 1)]:
