...
class testDB(ProxyDBTestCase):
  ...
  def test_getUsers(self):
    ...
    for table, values, fields in [('ProxyDB_Proxies',
                                  ['("user_1", "/C=DN/O=DIRAC/CN=user_1", "group_1", "PEM", TIMESTAMPADD(SECOND, 800, UTC_TIMESTAMP()))',
                                   '("user_2", "/C=DN/O=DIRAC/CN=user_2", "group_1", "PEM", TIMESTAMPADD(SECOND, -1, UTC_TIMESTAMP()))'],
                                  '(UserName, UserDN, UserGroup, Pem, ExpirationTime)'),
                                  ('ProxyDB_CleanProxies',
                                  ['("user_3", "/C=DN/O=DIRAC/CN=user_3", "PEM", TIMESTAMPADD(SECOND, 43200, UTC_TIMESTAMP()))'],
