...
conditions = {"SystemName": ["WorkloadManagement/Matcher", "Framework/ProxyManager"], "LogLevel": "ERROR"}
orderFields = [["OwnerDN", "ASC"], ["SystemName", "ASC"]]
retval = LClient.getGroupedMessages(
    fieldList=showFields,
    conds=conditions,
    beginDate="2008-09-18",
    endDate="2008-09-20",
...
retval = LClient.getGroupedMessages(groupField="FixedTextString", orderList=orderFields, maxRecords=10)
if not retval["OK"]:
    print(retval["Message"])
else:
    print(retval["Value"]["ParameterNames"])
