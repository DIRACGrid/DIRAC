...
class ComponentMonitoringClientChain(TestClientComponentMonitoring):
    ...
    def testComponents(self):
        ...
        result = self.client.addComponent(
            {"DIRACSystem": "Test", "DIRACModule": "TestModule", "Type": "TestingFeature"}
        )

        self.assertTrue(result["OK"])

        # Check if the component exists
        result = self.client.getComponents(
            {"DIRACSystem": "Test", "DIRACModule": "TestModule", "Type": "TestingFeature"}, False, False
        )

        self.assertTrue(result["OK"] and len(result["Value"]) > 0)

        # Update the fields of the created component
        result = self.client.updateComponents(
            {"DIRACSystem": "Test", "DIRACModule": "TestModule", "Type": "TestingFeature"},
        ...
        result = self.client.getComponents(
            {"DIRACSystem": "Test", "DIRACModule": "NewTestModule", "Type": "TestingFeature"}, False, False
        )

        self.assertTrue(result["OK"] and len(result["Value"]) > 0)

        # Remove the Component
        result = self.client.removeComponents(
            {"DIRACSystem": "Test", "DIRACModule": "NewTestModule", "Type": "TestingFeature"}
        )
        ...
        # Multiple removal
        self.client.addComponent({"DIRACSystem": "Test", "DIRACModule": "TestModule1", "Type": "TestingFeature1"})
        self.client.addComponent({"DIRACSystem": "Test", "DIRACModule": "TestModule2", "Type": "TestingFeature1"})
        self.client.addComponent({"DIRACSystem": "Test", "DIRACModule": "TestModule1", "Type": "TestingFeature2"})

        self.client.removeComponents({"DIRACSystem": "Test", "DIRACModule": "TestModule1"})

        result = self.client.getComponents(
            {"DIRACSystem": "Test", "DIRACModule": "TestModule2", "Type": "TestingFeature1"}, False, False
        )

        self.assertTrue(result["OK"] and len(result["Value"]) >= 1)

        result = self.client.getComponents({"DIRACSystem": "Test", "DIRACModule": "TestModule1"}, False, False)

        self.assertTrue(result["OK"] and len(result["Value"]) <= 0)

        self.client.removeComponents({"DIRACSystem": "Test", "DIRACModule": "TestModule2", "Type": "TestingFeature1"})
    ...
    def testHosts(self):
        ...
        result = self.client.getHosts({"HostName": "TestHost", "CPU": "TestCPU"}, False, False)

        self.assertTrue(result["OK"] and len(result["Value"]) > 0)

        # Update the fields of the created host
        result = self.client.updateHosts({"HostName": "TestHost", "CPU": "TestCPU"}, {"HostName": "StillATestHost"})

        self.assertTrue(result["OK"])

        # Check if the host with the modified fields exists
        result = self.client.getHosts({"HostName": "StillATestHost", "CPU": "TestCPU"}, False, False)

        self.assertTrue(result["OK"] and len(result["Value"]) > 0)

        # Remove the Host
        result = self.client.removeHosts({"HostName": "StillATestHost", "CPU": "TestCPU"})
        ...
        result = self.client.getHosts({"HostName": "TestHost", "CPU": "TestCPU2"}, False, False)

        self.assertTrue(result["OK"] and len(result["Value"]) >= 1)

        result = self.client.getHosts({"HostName": "TestHost", "CPU": "TestCPU1"}, False, False)
    ...
    def testInstallations(self):
        ...
        result = self.client.addInstallation(
            ...
            {"DIRACSystem": "UnexistentSystem", "DIRACModule": "UnexistentModule", "Type": "UnexistentType"},
            {"HostName": "fictional", "CPU": "TestCPU"},
            True,
        )
        ...
        result = self.client.updateInstallations(
            {"Instance": "TestInstallA111"},
            {"DIRACSystem": "UnexistentSystem", "DIRACModule": "UnexistentModule", "Type": "UnexistentType"},
            {"HostName": "fictional", "CPU": "TestCPU"},
        ...
        result = self.client.removeInstallations(
            {"Instance": "TestInstallA222"},
            {"DIRACSystem": "UnexistentSystem", "DIRACModule": "UnexistentModule", "Type": "UnexistentType"},
            {"HostName": "fictional", "CPU": "TestCPU"},
        )
        ...
        result = self.client.addInstallation(
            ...
            {"DIRACSystem": "UnexistentSystem", "DIRACModule": "UnexistentModule22A", "Type": "UnexistentType"},
            {"HostName": "fictional", "CPU": "TestCPU"},
            False,
        )
        ...
        self.client.addInstallation(
            ...
            {"DIRACSystem": "UnexistentSystem", "DIRACModule": "UnexistentModule2", "Type": "UnexistentType"},
            {"HostName": "fictional", "CPU": "TestCPU"},
            True,
        )

        result = self.client.getInstallations(
            {"Instance": ["MultipleRemovalInstall1", "MultipleRemovalInstall3"]}, {}, {}, False
        )

        self.assertTrue(result["OK"] and len(result["Value"]) == 2)

        self.client.removeInstallations({}, {"DIRACModule": "UnexistentModule"}, {})

        result = self.client.getInstallations({}, {"DIRACModule": "UnexistentModule2"}, {}, False)

        self.assertTrue(result["OK"] and len(result["Value"]) >= 1)

        result = self.client.getInstallations({}, {"DIRACModule": "UnexistentModule"}, {}, False)

        self.assertTrue(result["OK"] and len(result["Value"]) <= 0)

        self.client.removeInstallations({}, {"DIRACModule": "UnexistentModule2"}, {})
...
if __name__ == "__main__":
    ...
    suite.addTest(unittest.defaultTestLoader.loadTestsFromTestCase(ComponentMonitoringClientChain))
