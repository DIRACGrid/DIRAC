...
class ResourceManagementClientChain(TestClientResourceManagementTestCase):
    def test_AccountingCache(self):
        ...
        res = self.rmClient.addOrModifyAccountingCache(
    ...
    def test_DowntimeCache(self):
        ...
        res = self.rmClient.addOrModifyDowntimeCache(
            "TestName12345",
            "element",
            "name",
    ...
    def test_PolicyResult(self):
        ...
        res = self.rmClient.addOrModifyPolicyResult(
            "element", "TestName12345", "policyName", "statusType", status="newStatus"
        )
        self.assertTrue(res["OK"])
    ...
    def test_Transfer(self):
        ...
        res = self.rmClient.deleteTransferCache(
            "sourcename", "destinationname"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])

        # TEST addOrModifyTransferOccupancy
        res = self.rmClient.addOrModifyTransferCache(
            "sourcename", "destinationname", "metric", 1000.0, datetime.datetime.now()
        )
        self.assertTrue(res["OK"])
