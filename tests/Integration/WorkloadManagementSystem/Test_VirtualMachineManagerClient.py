...
class VirtualMachineManagerTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        res = self.__client.insertInstance(
            self.__inst_uuid, self.__inst_image, self.__inst_name, self.__inst_ep, self.__inst_pod
        )
        self.assertTrue(res["OK"])
        self.__id = res["Value"]
    ...
    def test_instanceProps(self):
        ...
        res = self.__client.instanceIDHeartBeat(
            self.__inst_uuid, INST_LOAD, INST_JOBS, INST_FILES, INST_BYTES, INST_UPTIME
        )
        self.assertTrue(res["OK"])
