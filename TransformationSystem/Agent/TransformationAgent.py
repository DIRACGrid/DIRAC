...
class TransformationAgent( AgentModule, TransformationAgentsUtilities ):
  ...
  def _execute( self ):
    ...
    while True:
      ...
      try:
        ...
        startTime = time.time()
        res = self.processTransformation( transDict, threadTransformationClient )
        if not res['OK']:
  ...
  def processTransformation( self, transDict, tsClient ):
    ...
    transFiles = self._getTransformationFiles( transDict, tsClient )
    ...
    res = self.__getDataReplicas( transID, lfns, tsClient, active = not replicateOrRemove )
    if not res['OK']:
    ...
    self._logInfo( "Processing transformation with '%s' plug-in." % plugin,
                    method = "processTransformation", transID = transID )
    res = self.__generatePluginObject( plugin, tsClient )
    if not res['OK']:
      return res
    oPlugin = res['Value']
    ...
    for se, lfns in tasks:
      res = tsClient.addTaskForTransformation( transID, lfns, se )
      if not res['OK']:
    ...
    if transDict['Status'] == 'Flush' and allCreated:
      res = tsClient.setTransformationParameter( transID, 'Status', 'Active' )
      if not res['OK']:
  ...
  def _getTransformationFiles( self, transDict, tsClient ):
    """ get the data replicas for a certain transID
    """

    transID = transDict['TransformationID']

    res = tsClient.getTransformationFiles( condDict = {'TransformationID':transID, 'Status':'Unused'} )
    if not res['OK']:
  ...
  def __generatePluginObject( self, plugin, tsClient ):
  ...
  def __getDataReplicas( self, transID, lfns, tsClient, active = True ):
    ...
    if newLFNs:
      self._logVerbose( "Getting replicas for %d files from catalog" % len( newLFNs ),
                         method = '__getDataReplicas', transID = transID )
      res = self.__getDataReplicasRM( transID, newLFNs, tsClient, active )
      if res['OK']:
  ...
  def __getDataReplicasRM( self, transID, lfns, tsClient, active = True ):
