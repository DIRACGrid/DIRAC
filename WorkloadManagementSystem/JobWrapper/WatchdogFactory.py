...
class WatchdogFactory( object ):
  ...
  #############################################################################
  def getWatchdog( self, pid, thread, spObject, jobcputime, jobArgs, memoryLimit, processors = 1 ):
...
#       self.log.info( 'WatchdogFactory will create Watchdog%s instance' % ( localOS ) )
    ...
    try:
      wd_o = getattr( wdModule, subClassName )( pid, thread, spObject, jobcputime, jobArgs, memoryLimit, processors )
      return S_OK( wd_o )
