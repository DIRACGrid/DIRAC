...
class WatchdogFactory( object ):
  ...
  #############################################################################
  def getWatchdog( self, pid, thread, spObject, jobCPUtime, memoryLimit, processors = 1, jobArgs = {} ):
...
#       self.log.info( 'WatchdogFactory will create Watchdog%s instance' % ( localOS ) )
    ...
    try:
      wd_o = getattr( wdModule, subClassName )( pid, thread, spObject, jobCPUtime, memoryLimit, processors, jobArgs = jobArgs )
      return S_OK( wd_o )
