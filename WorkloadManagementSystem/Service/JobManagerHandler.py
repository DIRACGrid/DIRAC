...
class JobManagerHandler(RequestHandler):
  ...
  def export_submitJob(self, jobDesc):
    ...
    for jobDescription in jobDescList:  # jobDescList because there might be a list generated by a parametric job
      result = self.gJobDB.insertNewJobIntoDB(jobDescription,
					      self.owner,
					      self.ownerDN,
					      self.ownerGroup,
					      self.diracSetup,
      ...
      self.gJobLoggingDB.addLoggingRecord(
...
###########################################################################
  ...
  def export_confirmBulkSubmission(self, jobIDs):
    ...
    result = self.gJobDB.getAttributesForJobList(
    ...
    result = self.gJobDB.setJobAttributes(jobUpdateStatusList,
					  ['Status', 'MinorStatus'],
...
###########################################################################
  ...
  def export_rescheduleJob(self, jobIDs):
    ...
    for jobID in validJobList:
      self.gTaskQueueDB.deleteJob(jobID)
      # gJobDB.deleteJobFromQueue(jobID)
      result = self.gJobDB.rescheduleJob(jobID)
      self.log.debug(str(result))
      if not result['OK']:
        return result
      self.gJobLoggingDB.addLoggingRecord(result['JobID'], result['Status'], result['MinorStatus'],
  ...
  def __deleteJob(self, jobID):
	...
	result = self.gPilotAgentsDB.getPilotInfo(pilotID=pilot)
        if not result['OK']:
          self.log.error("Failed to get pilot info", result['Message'])
          return result
        pilotRef = result[0]['PilotJobReference']
	ret = self.gPilotAgentsDB.deletePilot(pilot)
        if not ret['OK']:
          self.log.error("Failed to delete pilot from PilotAgentsDB", ret['Message'])
          return ret
	if self.gPilotsLoggingDB:
	  ret = self.gPilotsLoggingDB.deletePilotsLogging(pilotRef)
          if not ret['OK']:
  ...
  def __killJob(self, jobID, sendKillCommand=True):
    ...
    """
    if sendKillCommand:
      result = self.gJobDB.setJobCommand(jobID, 'Kill')
      if not result['OK']:
        return result

    self.log.info('Job marked for termination', jobID)
    result = self.gJobDB.setJobStatus(jobID, JobStatus.KILLED, 'Marked for termination')
    if not result['OK']:
  ...
  def __kill_delete_jobs(self, jobIDList, right):
    ...
    result = self.gJobDB.getAttributesForJobList(validJobList, ['Status'])
    if not result['OK']:
      return result
    killJobList = []
    deleteJobList = []
...
###########################################################################
  ...
  def export_resetJob(self, jobIDs):
    ...
    good_ids = []
    for jobID in validJobList:
      result = self.gJobDB.setJobAttribute(jobID, 'RescheduleCounter', -1)
      if not result['OK']:
        badIDs.append(jobID)
      else:
	self.gTaskQueueDB.deleteJob(jobID)
