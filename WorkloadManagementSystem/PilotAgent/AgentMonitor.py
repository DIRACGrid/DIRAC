...
#    self.pilotAgentsDB = PilotAgentsDB()
  ...
  def monitorPilot(self,jobID):
    ...
    """
    monResult = {}
    monResult['JobID']=jobID
    workingDir = '%s/%s' %(self.workingDirectory,jobID)
    monResult['WorkingDir']=workingDir
    ...
    if not atts['OK']:
      self.log.warn(result['Message'])
      self.__updateJobStatus(jobID,'Failed','JobDB Access Failure')
      atts['Value']=monResult
      return atts
    ...
    if not submittedAgents:
      self.__updateJobStatus(jobID,'Waiting','Pilot Agent Submission')
      result = S_ERROR('No submitted pilots found for job %s, updated to Pilot Agent Submission' % jobID)
      result['Value']=monResult
      return result
    ...
    if submittedAgents:
      self.__setJobParam(jobID,'SubmittedAgents',string.join(submittedAgents,','))
    if abortedAgents:
      self.__setJobParam(jobID,'AbortedAgents',string.join(abortedAgents,','))
  ...
  def reportResult(self,threadedJob,result):
    ...
    if result['OK']:
      self.log.info(result['Value']['Value'])
    else:
      self.log.warn('Error while monitoring job %s' %(jobID))
      self.log.warn(result['Message'])

    lock.acquire()
    if result['Value'].has_key('WorkingDir'):
  ...
  def reportException(self,threadedJob,result):
    ...
    self.log.warn('Exception reported while monitoring job')
    self.log.warn('Unpacking %s' %(result[2]))
    self.log.warn('------------>Traceback Starts')
    print_tb(result[2])
    self.log.warn('------------>Traceback Ends')
  ...
  def __updateJobStatus(self,job,status,minorstatus=None,logRecord=False):
    ...
    if result['OK']:
      if minorstatus:
        ...
        if self.enable:
          ...
          if not result['OK']:
            self.log.warn('Error updating job status:')
            self.log.warn(result['Message'])
        else:
          result = S_OK('DisabledMode')
    ...
    if self.enable:
      if logRecord:
        ...
        if not result['OK']:
          self.log.warn('Error adding logging record')
          self.log.warn(result['Message'])
  ...
  def __setJobParam(self,job,reportName,value):
    ...
    if self.enable:
      ...
      if not result['OK']:
        self.log.warn('Error adding job parameter')
        self.log.warn(result['Message'])
    else:
      result = S_OK('DisabledMode')
  ...
  def getPilotStatus(self,jobID,pilotID):
    ...
    """
    self.log.error('AgentMonitor: getPilotStatus method should be implemented in a subclass')
    return S_ERROR('AgentMonitor: getPilotStatus method should be implemented in a subclass')
