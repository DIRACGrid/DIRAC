########################################################################
# $Header: /tmp/libdirac/tmp.stZoy15380/dirac/DIRAC3/DIRAC/WorkloadManagementSystem/Client/Attic/ProxyManagerClient.py,v 1.1 2008/06/18 19:59:06 acasajus Exp $
########################################################################
""" ProxyManagementAPI has the functions to "talk" to the ProxyManagement service
"""
__RCSID__ = "$Id: ProxyManagerClient.py,v 1.1 2008/06/18 19:59:06 acasajus Exp $"
...
class ProxyManagerClient:
  ...
  def userHasProxy( self, userDN, userGroup, validSeconds = 0 ):
    ...
    if self.__usersCache.exists( cacheKey, validSeconds ):
      return S_OK( True )
    rpcClient = RPCClient( "WorkloadManagement/ProxyManager" )
  ...
  def uploadProxy( self, proxyLocation = False ):
    ...
    rpcClient = RPCClient( "WorkloadManagement/ProxyManager", proxyLocation = proxyLocation )
  ...
  def dumpProxyToFile( self, chain, requiredTimeLeft = 600 ):
    ...
    if self.__filesCache.exists( chain, requiredTimeLeft ):
      return S_OK( self.__filesCache.get( chain ) )
