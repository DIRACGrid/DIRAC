...
class PilotCStoJSONSynchronizer(object):
  ...
  def sync(self):
    ...
    # Can't do it in one step because ops.getValue would evaluate bool('False'),
    # which is True...
    uploadToWebApp = ops.getValue("Pilot/uploadToWebApp")
    if uploadToWebApp == "False":
      self.uploadToWebApp = False
